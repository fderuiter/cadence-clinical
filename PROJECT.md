...
- **Decoupling Rules**: Microservices strictly own their domain models under `apps/<service>/domain/`. No service may import database models or domain schemas from another service.
...
- **Anti-Corruption Layers (ACLs)**: Cross-service communication occurs via authenticated REST HTTP endpoints using HMAC-SHA256 V2 signatures (`generate_gateway_signature`). Incoming payloads are deserialized directly into local consumer-owned Pydantic DTOs under `apps/<service>/domain/acl/`.
...
- Consumer services deserialize JSON responses into local Pydantic DTOs defined in `apps/<service>/domain/acl/`.
