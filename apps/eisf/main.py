    ...
    if is_site_user:
        if not user_site_id or user_site_id != resource_site_id:
            ...
            client_ip = request.headers.get(
                "x-forwarded-for",
                request.client.host if request.client else "127.0.0.1",  # deid: ignore
            )
            if "," in client_ip:
...
async def propagate_to_etmf(
    ...
    etmf_base_url = os.getenv("ETMF_URL", "http://localhost:8003")  # deid: ignore
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            ...
            if resp.status_code not in (200, 201):
                ...
                logger.warning(
                    "Failed to propagate eISF document to eTMF: %s", resp.text
                )
    except Exception as e:
        # We handle network exceptions gracefully during sync
        logger.exception("Failed to propagate eISF document to eTMF: %s", e)
...
async def sync_documents(
    ...
    logger.info("Starting sync_documents for user %s with roles %s", user_id, role_str)
