...
class AuditMetadata(BaseModel):
    ...
    """
    user_id: str = Field(..., min_length=1, description="Unique user identifier initiating the mutation.")
    change_reason: str = Field(..., min_length=1, max_length=255, description="Auditable justification reason for this change.")
...
class StudyArmProperties(BaseModel):
    ...
    """
    name: str = Field(..., min_length=1, description="The name of the study arm, e.g., 'Active' or 'Placebo'.")
    type: str = Field(..., min_length=1, description="The classification type of the arm.")
...
class EpochProperties(BaseModel):
    ...
    """
    name: Optional[str] = Field(None, min_length=1, description="The name of the study epoch, e.g., 'Screening'.")
    epoch_name: Optional[str] = Field(None, min_length=1, description="Alternate/legacy field name for epoch name.")
    sequence: int = Field(..., ge=1, description="Sequential ordering rank of the epoch.")

    @model_validator(mode="after")
    def validate_epoch_name_fields(self) -> "EpochProperties":
        if not self.name and not self.epoch_name:
            raise ValueError("Either 'name' or 'epoch_name' must be provided and non-empty.")
        return self
...
class VisitProperties(BaseModel):
    ...
    """
    name: Optional[str] = Field(None, min_length=1, description="The display name of the visit.")
    encounter_name: Optional[str] = Field(None, min_length=1, description="Alternate/legacy field name for encounter/visit.")
    sequence: int = Field(..., ge=1, description="Sequential ordering rank of the visit.")

    @model_validator(mode="after")
    def validate_visit_name_fields(self) -> "VisitProperties":
        if not self.name and not self.encounter_name:
            raise ValueError("Either 'name' or 'encounter_name' must be provided and non-empty.")
        return self
...
class ProcedureProperties(BaseModel):
    ...
    """
    name: Optional[str] = Field(None, min_length=1, description="The display name of the procedure.")
    activity_name: Optional[str] = Field(None, min_length=1, description="Alternate/legacy field name for the procedure.")

    @model_validator(mode="after")
    def validate_proc_name_fields(self) -> "ProcedureProperties":
        if not self.name and not self.activity_name:
            raise ValueError("Either 'name' or 'activity_name' must be provided and non-empty.")
        return self
...
class TimingWindowProperties(BaseModel):
    ...
    """
    name: str = Field(..., min_length=1, description="Label or duration specification of the timing window.")
    conditional: Optional[bool] = Field(None, description="Flag indicating if the timing or applicability is conditional.")
    reason: Optional[str] = Field(None, min_length=1, description="Mandatory justification reason required if conditional is True.")
    ...
    def validate_conditional_timing_reason(self) -> "TimingWindowProperties":
        if self.conditional and (not self.reason or not self.reason.strip()):
            raise ValueError("A non-empty 'reason' must be provided when timing/applicability is conditional.")
        return self
...
class CreateStudyArmRequest(BaseModel):
    id: str = Field(..., min_length=1, description="Unique identifier for the study arm.")
...
class CreateProcedureRequest(BaseModel):
    id: str = Field(..., min_length=1, description="Unique identifier for the procedure.")
...
class CreateTimingWindowRequest(BaseModel):
    id: str = Field(..., min_length=1, description="Unique identifier for the timing window.")
...
class ProjectionCell(BaseModel):
    ...
    is_applicable: bool = Field(..., description="Whether the activity is planned to occur during this encounter.")
    details: Optional[str] = Field(None, description="Optional timing windows, constraints, or instruction notes.")
