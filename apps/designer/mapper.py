...
from packages.core_models.usdm import StudyDefinition, Arm, Visit, Activity, Concept
    ...
    for arm_data in study_data.get("arms", []):
        ...
        for visit_data in arm_data.get("visits", []):
            ...
            if "visit_type_concept_id" in visit_data:
                concept_data = terminology_cache.get(visit_data["visit_type_concept_id"])
                if concept_data:
