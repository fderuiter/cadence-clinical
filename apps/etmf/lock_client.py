...
async def verify_trial_lock_status(is_testing: Optional[bool] = None) -> bool:
    ...
    gateway_secret = gateway_secret_env.encode("utf-8") if isinstance(gateway_secret_env, str) else gateway_secret_env
...
async def trigger_global_trial_lock(reason: str, is_testing: Optional[bool] = None) -> None:
    """
    ...
    try:
        async with httpx.AsyncClient(timeout=5.0) as client:
            ...
            if response.status_code != 200:
                logger.error("Failed to set global trial lock, status code: %s", response.status_code)
                raise HTTPException(
                    status_code=502,
                    detail=f"Failed to set global trial lock: Execution service returned {response.status_code}"
                )
    except httpx.RequestError as e:
        logger.error("Failed to connect to execution service to trigger global trial lock: %s", e)
        raise HTTPException(
            status_code=502,
