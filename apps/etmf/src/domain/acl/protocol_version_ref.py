...
class ProtocolVersionRefDTO(BaseModel):
    ...
    Owned by eTMF service to prevent cross-service imports from Designer database or domain models.
    """
    ...
    def validate_study_id(cls, v: str) -> str:
        """Validate that study_id is a non-empty, non-blank string."""
    ...
    def validate_version_tag(cls, v: str) -> str:
        """Validate that version_tag is a non-empty, non-blank string."""
        if not isinstance(v, str) or not v.strip():
            raise ValueError(
    ...
    def validate_version_index(cls, v: int) -> int:
        """Validate that version_index is a positive integer >= 1."""
        if v < 1:
            raise ValueError("Version index must be a positive integer >= 1.")
        return v


# Class alias for backward compatibility across internal eTMF modules
