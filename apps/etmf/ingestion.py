...
async def ingest_document_service(
    ...
    Performs classification mapping, taxonomy resolution/validation, version-index allocation,
    immutable-status checks, document signature validation, and TMFDocument persistence,
    all wrapped in transactional boundaries with corresponding audit log registration.
    """
    ...
    # 6. Check if a document version already exists (for study_id + artifact_code)
    stmt = (
        select(TMFDocument)
