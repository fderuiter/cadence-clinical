...
async def ingest_tmf_document(
    ...
    *,
    ...
    from apps.etmf.application.classification_service import (
    ...
    if cert_pem and sig_b64:
        ...
        if "MOCK_SIGNATURE" in cert_pem:
            ...
            try:
                ...
                if cn_attr:
                    signer_name = str(cn_attr[0].value)
    ...
    if idempotency_key:
        ...
        if existing_idem:
            ...
            await session.flush()
            setattr(existing_idem, "_ingest_result_status", "ignored")
    ...
    if correlation_key and existing_doc:
        ...
        if is_redacted_case:
            ...
            session.add(log_entry)
            await session.flush()
            setattr(existing_doc, "_ingest_result_status", "ignored")
            return existing_doc
    ...
    try:
        async with session.begin_nested():
            ...
            setattr(doc, "_ingest_result_status", "created")
            return doc
