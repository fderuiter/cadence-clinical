...
describe("Subject Portal Service Worker (sw.js)", () => {
  ...
  beforeEach(() => {
    ...
    globalThis.Response = class Response {
      ...
      clone() {
        return new Response(this.body, { status: this.status, type: this.type });
      }
    };
    ...
    const runSW = new Function("self", "caches", "Response", "Request", "setTimeout", "clearTimeout", swCode);
    runSW(globalThis.self, globalThis.caches, globalThis.Response, globalThis.Request, setTimeout, clearTimeout);
  });
  ...
  it("should cleanup older caches on activation", async () => {
    ...
    expect(mockCaches.delete).not.toHaveBeenCalledWith("cadence-portal-cache-v1");
  });
  ...
  it("should bypass timeout and caching for non-static-asset GET requests (e.g. dynamic API queries)", async () => {
    ...
    const mockResponse = new globalThis.Response("{}", { status: 200, type: "cors" });
    globalThis.fetch = vi.fn().mockResolvedValue(mockResponse);
    
    const request = new globalThis.Request("http://localhost:8000/api/v1/assignments");
    const mockEvent = {
      request,
      respondWith: vi.fn(),
    };
  ...
  it("should handle static asset with network-first success within 2 seconds", async () => {
    ...
    const mockResponse = new globalThis.Response("index.html content", { status: 200, type: "basic" });
    globalThis.fetch = vi.fn().mockResolvedValue(mockResponse);
    
    const request = new globalThis.Request("http://localhost:5174/subject-portal/index.html");
    const mockEvent = {
      request,
      respondWith: vi.fn(),
    };
    ...
    expect(mockCaches.open).toHaveBeenCalledWith("cadence-portal-cache-v1");
    expect(mockCache.put).toHaveBeenCalledWith(request, expect.any(globalThis.Response));
  });
  ...
  it("should fallback to cache within 2 seconds if static asset request times out", async () => {
    ...
    const cachedResponse = new globalThis.Response("cached index.html", { status: 200 });
