"""Shared helper module for single-target and bulk SDV sign-off validation and upsert logic.
...
async def validate_and_upsert_sdv_target(
    ...
    Does NOT commit the database session.

    Returns:
        (True, signoff_db) on success
        (False, error_detail) on validation failure
    """
    ...
    if scope_str == "FIELD":
        ...
        if not obs_db:
            return False, "Clinical observation not found or inconsistent target/subject/study reference."
    elif scope_str == "VISIT":
        ...
        if not visit_db:
            return False, "Clinical visit not found or inconsistent target/subject/study reference."
    elif scope_str == "PAGE":
        ...
        if not res_page_obs.scalars().first():
            return False, "Page ID not found or inconsistent target/subject/study reference."
    else:
        return False, f"Invalid scope: {scope_str}"
