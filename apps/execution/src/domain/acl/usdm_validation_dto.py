"""Anti-Corruption Layer DTOs for USDM Validation Report in Execution Service."""
...
class USDMValidationDTO(BaseModel):
    """Local Execution ACL DTO representing USDM ingestion validation report."""
...
def normalize_usdm_payload(
    payload: dict[str, Any], target_version: str = "v2"
) -> dict[str, Any]:
    """Normalizes payload structure for target USDM version."""
    normalized = dict(payload)
    if "id" not in normalized and "study_id" in normalized:
        normalized["id"] = normalized["study_id"]
    return normalized
