...
async def test_trigger_rejects_missing_user_identifier(local_test_db):
    ...
    async with session_maker() as session:
        ...
        with pytest.raises(Exception) as exc_info:
            await conn.execute(
                text("INSERT INTO clinical_subjects (id, subject_id, study_id, status, version, is_deleted, is_unblinded) VALUES ('test-id', 'SUBJ-002', 'STUDY-XYZ', 'SCREENING', 1, 0, 0);")
            )
            await session.commit()
        
        assert "GxP Compliance Violation: Write operations lacking session-level user identifiers" in str(exc_info.value)
...
async def test_introspection_engine_excludes_compliance_tables(local_test_db, tmp_path):
    ...
    with pytest.raises(PermissionError):
        ...
        try:
            generate_typescript_schemas("postgresql://prod-db/clinical", str(output_file))
        finally:
