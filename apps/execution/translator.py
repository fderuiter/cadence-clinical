    ...
    with audit_context(user_id, change_reason):
        try:
            async with session_factory() as session:
                ...
                # Requirement 1: Commit the "PROCESSING" job status to the database in an isolated transaction
                # before executing any of the layout translation logic. This ensures the record is visible
                # to the system even if a crash occurs during the CPU-intensive translation.
                try:
                    ...
                    # If we cannot even initialize the job record, raise it.
                    raise init_err
                ...
                try:
                    ...
                    if "protocol" in payload and isinstance(payload["protocol"], dict):
                        if "documentedBy" in payload["protocol"]:
                            if isinstance(payload["protocol"]["documentedBy"], list):
                    ...
                    for item in raw_items:
                        ...
                        relevant_expr = (
                            " and ".join(
                                f"({r})" if len(relevants) > 1 else r for r in relevants
                            )
                            if relevants
                            else None
                        )
                        ...
                        for doc in docs:
                            ...
                            for nc in narrative_contents:
                                ...
                                if (
                                    ...
                                    or (nc_name and nc_name.lower() == item_id.lower())
                                ):
                            ...
                            for nc in narrative_contents:
                                nc_id = nc.get("id", "")
                                nc_name = nc.get("name", "")
                                if "hint" in nc_id.lower() or "hint" in nc_name.lower():
                                    if (
                                        orig_id and orig_id.lower() in nc_id.lower()
                                    ) or (
                                        orig_id and orig_id.lower() in nc_name.lower()
                                    ):
                                        hint_ncs.append(nc)
                                if "desc" in nc_id.lower() or "desc" in nc_name.lower():
                                    if (
                                        orig_id and orig_id.lower() in nc_id.lower()
                                    ) or (
                                        orig_id and orig_id.lower() in nc_name.lower()
                            ...
                            for nc in associated_ncs:
                                ...
                                if (
                                    ...
                                    desc = narrative_items[content_item_id].get("text")
                    ...
                    # Update the existing job record to COMPLETED in a new isolated transaction block
                    async with session.begin():
                    ...
                    # Update the existing job record to FAILED in a new isolated transaction block
                    async with session.begin():
                        db_job = await session.get(TranslationJob, actual_job_id)
                        if db_job:
