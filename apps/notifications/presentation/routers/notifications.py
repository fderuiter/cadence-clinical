"""FastAPI Router for Notifications microservice."""
...
async def poll_and_dispatch() -> None:
    """Polls the database for due notification deliveries and spawns tasks to process them."""
...
async def deliver_channel(delivery_id: str) -> None:
    ...
    from apps.notifications.application.delivery import (
...
@router.post(
    "/api/v1/notifications", response_model=NotificationResponse, status_code=201
)
...
async def list_notifications(
    ...
    """Retrieve notification records targeted to the current user's user ID or roles."""
    user_id = getattr(request.state, "user_id", "system")
    user_roles_list = get_normalized_roles(request)
    user_role = ",".join(user_roles_list) or "system"
...
@router.post(
    "/api/v1/notifications/{id}/acknowledge", response_model=NotificationResponse
)
...
@router.post("/api/v1/notifications/{id}/resolve", response_model=NotificationResponse)
