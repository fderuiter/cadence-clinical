...
async def test_webhook_delivery_channel_failure_and_retry_backoff():
    ...
    async with db_manager.get_session_maker()() as session:
        ...
        delivery_1.next_retry_at = datetime.now(UTC) - timedelta(seconds=1)
        await session.commit()
    ...
    async with db_manager.get_session_maker()() as session:
        ...
        delivery_2.next_retry_at = datetime.now(UTC) - timedelta(seconds=1)
        await session.commit()
...
async def test_multi_channel_edge_case_in_app_succeeds_email_exhausts():
    ...
    with patch("aiosmtplib.SMTP", return_value=mock_smtp_client):
        ...
        async with db_manager.get_session_maker()() as session:
            ...
            d_email.attempts = 4
            d_email.next_retry_at = datetime.now(UTC) - timedelta(seconds=1)
            await session.commit()
