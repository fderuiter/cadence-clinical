...
async def execute_quality_audit_sealing_cycle(
    ...
    last_block_query = await db.execute(
        text(
            "SELECT current_block_hash FROM quality_audit_ledger_seals ORDER BY block_index DESC LIMIT 1;"
        )
    )
    ...
    unsealed_query = await db.execute(
        text(
            "SELECT id, timestamp, user_id, user_role, action, details, record_id, change_reason "
            "FROM quality_audit_logs WHERE cryptographic_seal IS NULL ORDER BY timestamp ASC, id ASC LIMIT :limit;"
        ),
        {"limit": limit},
    )
    ...
    for rec in records:
        ...
        record_payload = {
            ...
            "change_reason": str(rec.change_reason) if rec.change_reason is not None else None,
        }
    ...
    await db.execute(
        text(
            "INSERT INTO quality_audit_ledger_seals (previous_block_hash, current_block_hash, timestamp, sealed_record_count, merkle_root_hash) "
    ...
    for rec_id in record_ids:
        await db.execute(
            text(
                "UPDATE quality_audit_logs SET cryptographic_seal = :seal WHERE id = :id;"
            ),
    ...
    try:
        ...
        seals_query = await db.execute(
            text(
                ...
                "FROM quality_audit_ledger_seals ORDER BY block_index ASC;"
            )
        )
        ...
        for seal in seals:
            ...
            records_query = await db.execute(
                text(
                    "SELECT id, timestamp, user_id, user_role, action, details, record_id, change_reason "
                    "FROM quality_audit_logs WHERE cryptographic_seal = :seal ORDER BY timestamp ASC, id ASC;"
                ),
        ...
        sealed_records_query = await db.execute(
            text(
                "SELECT DISTINCT cryptographic_seal FROM quality_audit_logs WHERE cryptographic_seal IS NOT NULL;"
            )
        )
        ...
        if orphan_seals:
            raise ValueError(f"Found orphan seals on Quality audit logs: {orphan_seals}")

        return True
    except Exception as e:
        TrialLockManager.lock_trial(reason=f"Quality GxP Data Integrity Breach: {str(e)}")
        raise ValueError(f"Quality GxP Data Integrity Breach: {str(e)}") from e
...
async def start_background_quality_sealer(
    ...
    async def sealer_loop():
        logger.info(
            "Background Quality ledger sealer started with interval %s seconds.", interval
        )
        while _should_run:
            try:
                async with session_maker() as db:
                    ...
                    if block_hash:
                        logger.info(
                            "Successfully sealed Quality block with hash: %s", block_hash
                        )
