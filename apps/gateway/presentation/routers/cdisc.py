...
async def list_cdisc_products(
    ...
    return [
        CdiscProductSummary(
            title="CDASH IG v2.3",
            version="2.3",
            description="Clinical Data Acquisition Standards Harmonies Implementation Guide",
        ),
        CdiscProductSummary(
            title="SDTM IG v3.4",
            version="3.4",
        ...
        CdiscProductSummary(
            ...
            version="2023-12-15",  # deid-ignore
...
async def get_controlled_terminology_codelist(
    ...
    cached = cache.get_codelist(codelist_code=codelist_code)
    if cached:
        return cached

    client = _get_cdisc_client()
    return await client.get_codelist(codelist_code=codelist_code)
