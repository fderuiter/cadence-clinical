...
export const terminologyClient = {
   ...
   * @param {Object} [options] - Optional identity or change reason configurations.
  ...
  async validateSingleCode(
    ...
    const url = `/api/v1/terminology/validate/${encodeURIComponent(code)}`;

    try {
      return await apiClient.get(url, { changeReason });
    } catch (error) {
   ...
   * @param {Object} [options] - Search configuration and identity options.
  ...
  async searchTerminology(
    ...
    const url = `/api/v1/terminology/search?${queryParams.toString()}`;
   ...
   * @param {Object} [options] - Gateway identity and change reason options.
  ...
  async getStudyTerminologyValidation(
    ...
    const url = `/api/v1/studies/${encodeURIComponent(studyId)}/terminology-validation`;
  ...
  async getStudyCtValidation(
    ...
    const url = `/api/v1/studies/${encodeURIComponent(studyId)}/ct-validation`;
