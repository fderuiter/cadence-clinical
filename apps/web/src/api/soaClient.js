 ...
 * Routes all requests through apiClient to use Keycloak Bearer authentication
 * and avoid browser construction of trusted identity/signature headers.
 */
export const soaClient = {
  ...
  async getSoAProjection(studyId, versionId, options = {}) {
    const changeReason = options.changeReason || "Get projection";
  ...
  async mutateEntity(
    ...
    const changeReason = options.changeReason;
  ...
  async batchSignOff(
    ...
    const changeReason = options.changeReason;
    const sigToken = options.sigToken;
    const headers = {
