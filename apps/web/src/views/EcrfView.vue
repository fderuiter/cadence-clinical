    ...
    <div class="grid-2">
      ...
      <div class="card">
        ...
        <form
          ...
          @submit.prevent
        >
          <!-- Concept Code Lookup Field for CT Live Validation Tests -->
          <div
            id="field-container-concept_code"
            class="clinical-lookup-container"
            style="grid-column: span 12; margin-bottom: 8px"
          >
            <label for="concept_code">Concept Code</label>
            <input
              id="concept_code"
              type="text"
              v-model="conceptCodeValue"
              @input="handleConceptCodeInput"
            />
            <div
              v-if="conceptCodeStatus !== 'none'"
              id="lookup-status-concept_code"
              class="lookup-status"
              :class="conceptCodeStatusClass"
              style="margin-top: 4px; font-size: 0.85rem"
            >
              {{ conceptCodeStatusText }}
            </div>
          </div>
...
const conceptCodeValue = ref("");
const conceptCodeStatus = ref("none");
const conceptCodeStatusText = ref("");
const conceptCodeStatusClass = ref("");

let conceptCodeDebounceTimer = null;
let conceptCodeRequestSeq = 0;

function handleConceptCodeInput() {
  conceptCodeStatus.value = "loading";
  conceptCodeStatusClass.value = "lookup-loading";
  conceptCodeStatusText.value = "Searching...";

  if (conceptCodeDebounceTimer) {
    clearTimeout(conceptCodeDebounceTimer);
  }

  const currentSeq = ++conceptCodeRequestSeq;

  conceptCodeDebounceTimer = setTimeout(async () => {
    try {
      const res = await terminologyClient.validateSingleCode(
        conceptCodeValue.value,
        {}
      );
      if (currentSeq !== conceptCodeRequestSeq) return; // Stale response guard

      if (res.state === "VALID") {
        conceptCodeStatus.value = "valid";
        conceptCodeStatusClass.value = "lookup-valid";
        conceptCodeStatusText.value = `Code is valid: "${res.decode}"`;
      } else if (res.state === "INVALID") {
        conceptCodeStatus.value = "invalid";
        conceptCodeStatusClass.value = "lookup-invalid";
        conceptCodeStatusText.value = `Invalid code`;
      } else if (res.state === "DEGRADED") {
        conceptCodeStatus.value = "degraded";
        conceptCodeStatusClass.value = "lookup-degraded";
        conceptCodeStatusText.value = `Terminology service degraded. Validation offline.`;
      }
    } catch {
      if (currentSeq !== conceptCodeRequestSeq) return;
      conceptCodeStatus.value = "degraded";
      conceptCodeStatusClass.value = "lookup-degraded";
      conceptCodeStatusText.value = `Terminology service degraded. Validation offline.`;
    }
