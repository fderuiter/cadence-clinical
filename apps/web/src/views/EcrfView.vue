        ...
        >
          <template v-for="field in store.ecrfFields" :key="field.id">
            ...
            >
                ...
                <input
                  ...
                  @input="
                    field.type === 'concept_code'
                      ? handleConceptCodeInput(field, $event.target.value)
                      : null
                  "
                  @change="
              ...
              <!-- Status Indicator for concept_code fields -->
              <div
                v-if="
                  field.type === 'concept_code' &&
                  conceptCodeStatus.state !== 'none'
                "
                :id="`lookup-status-${field.id}`"
                class="lookup-status-indicator"
                :class="getLookupStatusClass"
                role="status"
                aria-live="polite"
              >
...
// Concept code lookup reactive status states
const conceptCodeStatus = ref({
  state: "none", // none, loading, valid, invalid, degraded
  message: "",
  version: 0,
});

const getLookupStatusClass = computed(() => {
  if (conceptCodeStatus.value.state === "loading") return "lookup-loading";
  if (conceptCodeStatus.value.state === "valid") return "lookup-valid";
  if (conceptCodeStatus.value.state === "invalid") return "lookup-invalid";
  if (conceptCodeStatus.value.state === "degraded") return "lookup-degraded";
  return "";
});

const getLookupStatusIcon = computed(() => {
  if (conceptCodeStatus.value.state === "loading") return "⏳";
  if (conceptCodeStatus.value.state === "valid") return "✅";
  if (conceptCodeStatus.value.state === "invalid") return "❌";
  if (conceptCodeStatus.value.state === "degraded") return "⚠️";
  return "";
});

let conceptCodeTimer = null;
let conceptCodeRequestCount = 0;

function handleConceptCodeInput(field, val) {
  store.formValues[field.id] = val;

  if (conceptCodeTimer) {
    clearTimeout(conceptCodeTimer);
  }

  if (!val.trim()) {
    conceptCodeStatus.value = {
      state: "none",
      message: "",
      version: conceptCodeRequestCount,
    };
    return;
  }

  conceptCodeStatus.value.state = "loading";
  conceptCodeStatus.value.message = "Searching terminology database...";

  conceptCodeRequestCount++;
  const currentRequestVersion = conceptCodeRequestCount;

  conceptCodeTimer = setTimeout(async () => {
    try {
      const res = await terminologyClient.validateSingleCode(val, {});

      // Guard against race conditions / stale responses
      if (currentRequestVersion < conceptCodeRequestCount) {
        return;
      }
      ...
      if (res.state === "VALID") {
        ...
        conceptCodeStatus.value = {
          state: "degraded",
          message: res.error_message || "service degraded",
          version: currentRequestVersion,
        };
      }
    } catch {
