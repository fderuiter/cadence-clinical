<template>
  <div id="section-rules" class="dashboard-section active">
    <div class="section-header">
      ...
      <p>
        Declarative visual rule builder supporting skip logic, constraint checks, and cross-form edit validations.
      </p>
    </div>

    <!-- Authorization Gating check -->
    <div v-if="!hasEditAccess" class="card" style="border-left: 4px solid var(--error); background-color: var(--error-bg); padding: 24px;">
      <div style="display: flex; gap: 16px; align-items: flex-start;">
        <span style="font-size: 2rem;">🚫</span>
        <div>
          <h3 style="color: var(--error); font-weight: bold; margin-bottom: 8px;">21 CFR Part 11 Role Gating - Access Denied</h3>
          <p style="color: var(--neutral-dark); font-size: 0.95rem; line-height: 1.6;">
            You do not have the required <strong>STUDY_DESIGNER</strong> role to view or interact with the clinical study rules.
            Please authenticate with an authorized clinical design token or consult your system administrator.
          </p>
        </div>
      </div>
    </div>
    ...
    <div v-else class="grid-2" style="display: grid; grid-template-columns: 1fr 1fr; gap: 24px;">
      ...
      <div class="card" style="display: flex; flex-direction: column; height: fit-content;">
        <div class="card-title" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px;">
          ...
          <span v-if="loadingRules" style="font-size: 0.85rem; color: #64748b; font-weight: normal;">Loading...</span>
        </div>

        <!-- Connection Error Banner if any -->
        <div v-if="connectionError" style="background-color: var(--warning-bg); border: 1px solid var(--warning); color: var(--warning); padding: 10px; border-radius: 6px; margin-bottom: 12px; font-size: 0.85rem;">
        ...
        <div style="flex: 1; max-height: 600px; overflow-y: auto; margin-bottom: 16px; padding-right: 4px;">
          <div v-if="activeRules.length === 0" style="color: #64748b; font-style: italic; padding: 12px 0; text-align: center;">
            No active rules configured. Click "Create New Rule" to get started.
          </div>
          <div v-else>
            <div v-for="rule in activeRules" :key="rule.id" class="rule-card" style="border: 1px solid var(--border); border-radius: 8px; padding: 16px; margin-bottom: 12px; background-color: var(--neutral-light); transition: all 0.2s;">
              <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 8px;">
                <strong style="color: var(--accent); font-size: 0.95rem; font-family: monospace;">{{ rule.id }}</strong>
                <div style="display: flex; gap: 6px;">
                  <button class="btn" style="padding: 4px 10px; font-size: 0.75rem; cursor: pointer; border: 1px solid var(--border); border-radius: 4px; background: white;" @click="openRuleEditor(rule)">Edit</button>
                  <button class="btn" style="padding: 4px 10px; font-size: 0.75rem; background-color: var(--error); color: white; border: none; border-radius: 4px; cursor: pointer;" @click="promptDeleteRule(rule.id)">Delete</button>
                </div>
              </div>
              <div style="font-size: 0.85rem; color: var(--neutral-dark); margin-bottom: 8px; line-height: 1.4;">
                <div style="margin-bottom: 4px;"><strong>Type:</strong> <span class="badge" style="background-color: var(--primary-light); color: white; font-size: 0.7rem; padding: 2px 6px;">{{ rule.type }}</span></div>
                <div v-if="rule.type === 'skip_logic'">
                  <strong>Action:</strong> {{ rule.action }} field <code>{{ rule.target_field }}</code>
                </div>
                <div v-else-if="rule.type === 'constraint'">
                  <strong>Target Field:</strong> <code>{{ rule.target_field }}</code> <br/>
                  <strong>Discrepancy Message:</strong> <span style="font-style: italic; color: var(--primary);">"{{ rule.query_message }}"</span>
                </div>
                <div v-else-if="rule.type === 'cross_form_check'">
                  <strong>Discrepancy Message:</strong> <span style="font-style: italic; color: var(--primary);">"{{ rule.query_message }}"</span>
                </div>
              </div>
              <div style="font-family: monospace; font-size: 0.75rem; color: #0284c7; background-color: #f0f9ff; padding: 6px 10px; border-radius: 4px; border: 1px solid #e0f2fe; word-break: break-all;">
                <strong>XPath:</strong> {{ rule.compiled_xpath || rule.xpath || "(Not compiled)" }}
              </div>
            </div>
          </div>
        </div>
        ...
        <div style="display: flex; gap: 8px;">
          <button class="btn btn-primary" style="width: 100%; display: flex; align-items: center; justify-content: center; gap: 8px;" @click="openRuleEditor(null)">
      ...
      <div v-if="showEditor" class="card" style="display: flex; flex-direction: column;">
        ...
        <div style="display: flex; flex-direction: column; gap: 16px; overflow-y: auto; max-height: 700px; padding-right: 4px;">
          ...
          <fieldset style="border: 1px solid var(--border); border-radius: 8px; padding: 16px;">
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px; margin-bottom: 16px;">
              <div class="form-group">
                ...
                <select v-model="ruleType" @change="handleTypeChange" style="width: 100%; padding: 8px; border: 1px solid var(--border); border-radius: 6px;">
              ...
              <div v-if="ruleType !== 'cross_form_check'" class="form-group">
                <label style="display: block; margin-bottom: 6px; font-weight: 600; font-size: 0.85rem;">Target Field</label>
                <select v-model="targetField" style="width: 100%; padding: 8px; border: 1px solid var(--border); border-radius: 6px;">
                  <option value="">-- Select Target Field --</option>
                  <option v-for="f in mockStudyFields" :key="f.id" :value="f.id">{{ f.name }}</option>
                </select>
              </div>
            </div>
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px;">
              <div v-if="ruleType === 'skip_logic'" class="form-group">
                <label style="display: block; margin-bottom: 6px; font-weight: 600; font-size: 0.85rem;">Action on Target</label>
                <select v-model="ruleAction" style="width: 100%; padding: 8px; border: 1px solid var(--border); border-radius: 6px;">
              ...
              <div v-if="ruleType === 'skip_logic'" class="form-group">
                <label style="display: block; margin-bottom: 6px; font-weight: 600; font-size: 0.85rem;">Target Form (Optional)</label>
                <select v-model="targetForm" style="width: 100%; padding: 8px; border: 1px solid var(--border); border-radius: 6px;">
                  <option value="">-- Select Target Form --</option>
                  <option v-for="f in mockStudyForms" :key="f.id" :value="f.id">{{ f.name }}</option>
                </select>
              </div>

              <div v-if="ruleType !== 'skip_logic'" class="form-group" style="grid-column: span 2;">
                <label style="display: block; margin-bottom: 6px; font-weight: 600; font-size: 0.85rem;">Auto-Query Discrepancy Message</label>
                <input type="text" v-model="queryMessage" placeholder="e.g., Systolic BP is out of logical range" style="width: 100%; padding: 8px; border: 1px solid var(--border); border-radius: 6px;" />
              </div>
            </div>
          </fieldset>
          ...
          <fieldset style="border: 1px solid var(--border); border-radius: 8px; padding: 16px;">
              ...
              <fieldset v-for="(cond, index) in conditions" :key="index" style="border: 1px dashed var(--border); border-radius: 8px; padding: 12px; background-color: #fafbfd;">
                ...
                <div style="display: flex; gap: 8px; align-items: flex-end; flex-wrap: wrap;">
                  <div class="form-group" style="flex: 1; min-width: 100px;">
                    ...
                    <select v-model="cond.formId" style="width: 100%; padding: 6px; font-size: 0.8rem; border-radius: 4px; border: 1px solid var(--border);">
                  ...
                  <div class="form-group" style="flex: 1; min-width: 100px;">
                    <label style="font-size: 0.75rem; display: block; margin-bottom: 4px;">Left Field</label>
                    <select v-model="cond.fieldId" style="width: 100%; padding: 6px; font-size: 0.8rem; border-radius: 4px; border: 1px solid var(--border);">
                  ...
                  <div class="form-group" style="flex: 1; min-width: 100px;">
                    <label style="font-size: 0.75rem; display: block; margin-bottom: 4px;">Operator</label>
                    <select v-model="cond.operator" style="width: 100%; padding: 6px; font-size: 0.8rem; border-radius: 4px; border: 1px solid var(--border);">
                  ...
                  <div v-if="cond.operator !== 'is_empty' && cond.operator !== 'is_not_empty'" class="form-group" style="flex: 1; min-width: 100px;">
                    <label style="font-size: 0.75rem; display: block; margin-bottom: 4px;">Right Value Type</label>
                    <select v-model="cond.rightType" style="width: 100%; padding: 6px; font-size: 0.8rem; border-radius: 4px; border: 1px solid var(--border);">
                      <option value="constant">Constant Value</option>
                      <option value="field_ref">Field Reference</option>
                    </select>
                  </div>

                  <div v-if="cond.operator !== 'is_empty' && cond.operator !== 'is_not_empty' && cond.rightType === 'constant'" class="form-group" style="flex: 1; min-width: 100px;">
                    <label style="font-size: 0.75rem; display: block; margin-bottom: 4px;">Constant Value</label>
                    <input type="text" v-model="cond.rightValue" placeholder="Value..." style="width: 100%; padding: 6px; font-size: 0.8rem; border-radius: 4px; border: 1px solid var(--border);" />
                  </div>

                  <div v-if="cond.operator !== 'is_empty' && cond.operator !== 'is_not_empty' && cond.rightType === 'field_ref'" class="form-group" style="flex: 1; min-width: 100px;">
                    <label style="font-size: 0.75rem; display: block; margin-bottom: 4px;">Right Field</label>
                    <select v-model="cond.rightFieldId" style="width: 100%; padding: 6px; font-size: 0.8rem; border-radius: 4px; border: 1px solid var(--border);">
                      <option value="">-- Select Field --</option>
                      <option v-for="f in mockStudyFields" :key="f.id" :value="f.id">{{ f.name }}</option>
                    </select>
                  </div>

                  <button class="btn" style="background-color: var(--error); color: white; border: none; padding: 6px 12px; border-radius: 4px; cursor: pointer; font-size: 0.8rem;" @click="removeConditionRow(index)">
                    Remove
                  </button>
                </div>
              </fieldset>
            ...
            <div style="margin-top: 12px;">
              <button class="btn btn-secondary" style="padding: 6px 12px; font-size: 0.8rem; cursor: pointer;" @click="addConditionRow">
                ➕ Add Condition Row
              </button>
            </div>
          </fieldset>
          ...
          <fieldset style="border: 1px solid var(--border); border-radius: 8px; padding: 16px;">
            <legend style="padding: 0 8px; font-weight: bold; color: var(--accent);">Live Compilation & GxP Verification Preview</legend>
            <div style="background-color: #f1f5f9; padding: 12px; border-radius: 8px; font-family: monospace; font-size: 0.8rem; border: 1px solid var(--border); line-height: 1.5;">
              <div style="font-weight: 700; color: var(--primary); margin-bottom: 4px;">Compiled XPath Expression:</div>
              <div style="color: #0369a1; margin-bottom: 12px; word-break: break-all; min-height: 1.5em;">{{ previewXpath || '(No conditions added)' }}</div>

              <div style="font-weight: 700; color: var(--primary); margin-bottom: 4px;">Validation Feedback:</div>
              <div v-if="previewFailures.length > 0" style="color: var(--error); font-weight: 600; margin-bottom: 8px;">
                <div v-for="(f, i) in previewFailures" :key="i">⚠️ {{ f }}</div>
              </div>
              <div v-else style="color: var(--success); font-weight: 600; margin-bottom: 8px;">✅ CDASH Field & Pydantic Schema Aligned.</div>

              <div v-if="previewCircularCycles.length > 0" style="color: var(--error); font-weight: 600;">
                <div v-for="(c, i) in previewCircularCycles" :key="i">🚨 {{ c }}</div>
              </div>
            </div>
          </fieldset>
        ...
        <div style="margin-top: 20px; display: flex; justify-content: flex-end; gap: 8px; border-top: 1px solid var(--border); padding-top: 16px;">
          <button class="btn" style="padding: 8px 16px; cursor: pointer;" @click="showEditor = false">Cancel</button>
          <button class="btn btn-primary" style="padding: 8px 16px; cursor: pointer;" @click="promptSaveRule">Save Signed Rule</button>
        </div>
      </div>
    ...
    <div v-if="showReasonModal" class="modal-overlay" style="display: flex; position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0,0,0,0.5); justify-content: center; align-items: center; z-index: 1000;">
      <div class="modal" style="background: white; border-radius: 12px; max-width: 500px; width: 100%; padding: 24px; box-shadow: 0 10px 25px rgba(0,0,0,0.25);">
        <div class="modal-header" style="font-size: 1.2rem; font-weight: bold; color: var(--primary); margin-bottom: 12px; border-bottom: 1px solid var(--border); padding-bottom: 8px;">Reason for Change Required</div>
        <div class="modal-body" style="font-size: 0.9rem; color: var(--neutral-dark); line-height: 1.5; margin-bottom: 16px;">
          <p style="margin-bottom: 12px;">
            To comply with <strong>21 CFR Part 11 / EU Annex 11</strong>, you must document a reason for changing this clinical data check rule.
          </p>
          <div class="form-group" style="margin-bottom: 12px;">
            <label style="display: block; margin-bottom: 4px; font-weight: 600;">Select Standard Reason</label>
            <select v-model="changeReason" style="width: 100%; padding: 8px; border: 1px solid var(--border); border-radius: 6px;">
              <option value="Initial Entry">Initial Data Entry</option>
              <option value="Typographical Error">Correction of typographical error</option>
              <option value="Re-measurement">Re-measurement of vitals</option>
              <option value="Transcription Error">Correction of transcription error</option>
              <option value="Protocol Amendment">Protocol Amendment / Update</option>
              <option value="Other">Other (specify below)</option>
            </select>
          </div>
          <div class="form-group">
            <label style="display: block; margin-bottom: 4px; font-weight: 600;">Custom Explanation (Optional)</label>
            <textarea v-model="customChangeReason" placeholder="Explain the clinical reason for this modification..." style="width: 100%; height: 80px; padding: 8px; border: 1px solid var(--border); border-radius: 6px; resize: none;"></textarea>
          </div>
        </div>
        <div class="modal-footer" style="display: flex; justify-content: flex-end; gap: 8px; border-top: 1px solid var(--border); padding-top: 12px;">
          <button class="btn" style="padding: 6px 16px; cursor: pointer;" @click="closeReasonModal">Cancel Change</button>
          <button class="btn btn-primary" style="padding: 6px 16px; cursor: pointer;" @click="confirmChangeReason">Sign & Save</button>
        </div>
      </div>
    </div>
  </div>
...
async function fetchRules() {
  ...
  try {
    ...
    const response = await fetch(`${GATEWAY_URL}/api/v1/studies/study_1/rules`, { headers });
    if (!response.ok) throw new Error("REST API Fetch error");
    activeRules.value = await response.json();
  } catch (err) {
    console.warn("Failed to fetch active rules from REST backend, falling back to empty:", err);
...
async function triggerPreview() {
  const payload = {
    ...
    target_field: ruleType.value !== "cross_form_check" ? targetField.value || null : null,
    target_form: ruleType.value === "skip_logic" ? targetForm.value || null : null,
    action: ruleType.value === "skip_logic" ? ruleAction.value : null,
    query_message: ruleType.value !== "skip_logic" ? queryMessage.value || null : null,
  };

  try {
    const headers = await getSignedHeaders("Rule compilation preview");
    const response = await fetch(`${GATEWAY_URL}/api/v1/studies/study_1/rules/preview`, {
      method: "POST",
      headers,
    ...
    console.warn("Dry-run preview API failed, performing fallback local compilation:", err);
...
function compileLocalFallback(payload) {
  ...
  if (payload.type === "skip_logic" && payload.target_field) {
    ...
    if (refs.includes(payload.target_field)) {
      previewCircularCycles.value.push(`Circular dependency: ${payload.target_field} -> ${payload.target_field}`);
    }
  }
}
...
async function confirmChangeReason() {
  const reasonText = (changeReason.value === "Other" && customChangeReason.value)
    ? customChangeReason.value
    : changeReason.value;

  if (!reasonText || !reasonText.trim()) {
    alert("Compliance modification justification reason text is strictly required!");
    return;
  }
  ...
  try {
    if (action.type === "save") {
      ...
      if (!response.ok) {
        ...
        throw new Error(err.detail || "Server failed to compile or authorize rule.");
      }
      ...
      const headers = await getSignedHeaders(reasonText);
      const response = await fetch(`${GATEWAY_URL}/api/v1/studies/study_1/rules/${action.ruleId}`, {
        method: "DELETE",
        headers,
      });
