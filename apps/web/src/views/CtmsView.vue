<template>
  ...
  >
    <div class="section-header">
      ...
      <p>
        Monitor operational site status, track site lifecycle milestones, recruitment metrics, and coordinate CRA workloads.
      </p>
    </div>
...
function achieveMilestone() {
  const nextM = store.currentCtmsData.milestones.find((m) => m.status === "PLANNED");
  if (nextM) {
...
function completeVisit() {
  const scheduledVisit = store.currentCtmsData.visits.find((v) => v.status === "SCHEDULED");
  if (scheduledVisit) {
...
function reallocateCra() {
  const craAlice = store.currentCtmsData.allocations.find((a) => a.cra === "cra_alice");
  if (craAlice) {
...
function updateRecruitment() {
  const site1 = store.currentCtmsData.recruitment.find((r) => r.siteId === "Site-01");
  if (site1) {
    site1.screened += 2;
    site1.enrolled += 1;
    store.addLedgerBlock(
