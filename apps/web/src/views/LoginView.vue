<template>
  ...
  >
    <div class="section-header" style="text-align: center; margin-bottom: 24px;">
      ...
      <p style="color: #64748b;">Identity Gateway and Security Assertion Portal</p>
    </div>

    <!-- MAIN CONTAINER CARD -->
    <div class="card" style="border: 1px solid #e2e8f0; border-radius: 8px; box-shadow: 0 4px 6px -1px rgb(0 0 0 / 0.1); background: #fff; overflow: hidden;">
      <div
        class="card-title"
        style="justify-content: center; font-size: 1.25rem; padding: 16px; border-bottom: 1px solid #e2e8f0; background-color: #f8fafc; text-align: center; font-weight: 600;"
      >
      ...
      <div style="padding: 24px;">
        ...
        >
          ...
          <p style="font-size: 0.9rem; color: #64748b; font-weight: normal; margin-bottom: 20px;">
        ...
        <div v-else-if="showPinSetup" style="max-width: 400px; margin: 0 auto;">
          <h3 style="margin-bottom: 12px; text-align: center; color: #1e293b;">Set Up Personalized Recovery PIN</h3>
          <p style="font-size: 0.875rem; color: #475569; margin-bottom: 20px; text-align: center;">
            This 4-to-6 digit PIN wraps your cryptographic escrow key client-side, securing offline submission queues for multi-user tablets.
          </p>

          <div style="margin-bottom: 16px;">
            <label style="display: block; font-size: 0.875rem; font-weight: 600; margin-bottom: 6px; color: #334155;">
            ...
            <input
              ...
              placeholder="••••"
              class="form-control"
              style="width: 100%; padding: 10px; border: 1px solid #cbd5e1; border-radius: 4px; box-sizing: border-box; text-align: center; font-size: 1.25rem; letter-spacing: 0.25em;"
          ...
          <div v-if="pinSetupError" style="color: #dc2626; font-size: 0.875rem; margin-bottom: 16px; text-align: center; font-weight: 600;">
            {{ pinSetupError }}
          </div>
          ...
          <div style="display: flex; gap: 12px;">
            <button
              ...
              style="flex: 1; padding: 10px; cursor: pointer; background: #f1f5f9; border: 1px solid #cbd5e1; color: #475569; border-radius: 4px;"
              @click="cancelPinFlow"
            >
              Cancel
            </button>
            <button
              class="btn btn-primary"
              style="flex: 1; padding: 10px; cursor: pointer; font-weight: 600; border-radius: 4px;"
        ...
        <div v-else-if="showPinUnlock" style="max-width: 400px; margin: 0 auto;">
          <h3 style="margin-bottom: 12px; text-align: center; color: #1e293b;">Unlock Session Key Escrow</h3>
          <p style="font-size: 0.875rem; color: #475569; margin-bottom: 20px; text-align: center;">
            A dormant offline queue is protected by a PIN-wrapped key. Enter PIN to decrypt and process.
          </p>
          ...
          <div v-if="unlockError" style="color: #dc2626; font-size: 0.875rem; margin-bottom: 16px; text-align: center; font-weight: 600;">
            {{ unlockError }}
          </div>
          ...
          <div style="display: flex; flex-direction: column; gap: 10px;">
            <button
              ...
              style="width: 100%; padding: 12px; font-weight: 600; cursor: pointer; border-radius: 4px;"
            ...
            <button
              class="btn btn-secondary"
              style="width: 100%; padding: 10px; cursor: pointer; background: #f1f5f9; border: 1px solid #cbd5e1; color: #475569; border-radius: 4px;"
              @click="cancelPinFlow"
            >
              Back to Main Login
            </button>
            ...
            <button
              ...
              style="width: 100%; padding: 10px; cursor: pointer; background: #fee2e2; border: 1px solid #fca5a5; color: #b91c1c; border-radius: 4px; margin-top: 10px;"
        ...
        <div v-else>
          <p style="margin-bottom: 24px; color: #475569; font-size: 0.95rem; text-align: center;">
...
async function checkUserLockStatus() {
  ...
  const metadata = await offlineAuthManager.getSessionMetadata(selectedUserToUnlock.value);
  if (metadata) {
...
async function submitPinSetup() {
  ...
  try {
    if (pendingLoginType.value === "keycloak") {
      ...
      await authStore.login();
      const identity = authStore.identity || { id: "demo-user-id", username: "fderuiter" };
      const session = {
        userId: identity.id,
        userRoles: authStore.normalizedRoles,
...
async function submitPinUnlock() {
  ...
  try {
    const session = await offlineAuthManager.unlockOfflineSession(unlockPin.value, selectedUserToUnlock.value);
...
async function purgeUserEscrow() {
  if (!selectedUserToUnlock.value) return;
  if (confirm(`Are you sure you want to completely delete the offline key escrow and sync queue for ${selectedUserToUnlock.value}? This cannot be undone.`)) {
