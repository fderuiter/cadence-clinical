...
export function normalizeUsdm(rawData: any): NormalizedUsdm {
  ...
  const studyId = typeof data.studyId === "string" ? data.studyId : "STUDY-USDM-001";
  ...
  const encounters: SoAHeaderEncounter[] = rawEncounters.map((enc: any): SoAHeaderEncounter => {
    return {
      encounter_id: typeof enc.encounter_id === "string" ? enc.encounter_id : "",
      encounter_name: typeof enc.encounter_name === "string" ? enc.encounter_name : "",
  ...
  const rows: SoARowView[] = rawRows.map((row: any): SoARowView => {
    ...
    const cells: SoACellView[] = rawCells.map((cell: any): SoACellView => {
      return {
        activity_id: typeof cell.activity_id === "string" ? cell.activity_id : actId,
        encounter_id: typeof cell.encounter_id === "string" ? cell.encounter_id : "",
        epoch_id: typeof cell.epoch_id === "string" ? cell.epoch_id : "",
        is_applicable: typeof cell.is_applicable === "boolean" ? cell.is_applicable : false,
        details: typeof cell.details === "string" ? cell.details : "",
        arm_id: cell.arm_id || null,
        derived_from_soa: typeof cell.derived_from_soa === "boolean" ? cell.derived_from_soa : false,
      };
    });

    return {
      activity_id: actId,
      activity_name: typeof row.activity_name === "string" ? row.activity_name : "",
      cells,
      derived_from_soa: typeof row.derived_from_soa === "boolean" ? row.derived_from_soa : false,
    };
  });
