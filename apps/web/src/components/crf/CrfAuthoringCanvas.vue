<template>
  <div class="authoring-canvas-layout flex flex-col lg:flex-row gap-6">
    ...
    <div class="flex-1 flex flex-col gap-4">
      <div class="canvas-header flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 bg-white p-4 rounded-xl border border-slate-200">
        <div>
        ...
        <div class="flex items-center gap-4 flex-wrap">
          ...
          <div class="viewport-selector flex bg-slate-100 p-1 rounded-lg border border-slate-200">
            <button
              class="btn-viewport-desktop px-3 py-1.5 text-xs font-semibold rounded-md transition-all flex items-center gap-1"
              :class="designerStore.viewport === 'desktop' ? 'bg-indigo-600 text-white shadow-sm' : 'text-slate-600 hover:text-slate-800'"
            ...
            <button
              ...
              :class="designerStore.viewport === 'tablet' ? 'bg-indigo-600 text-white shadow-sm' : 'text-slate-600 hover:text-slate-800'"
            ...
            <button
              ...
              :class="designerStore.viewport === 'mobile' ? 'bg-indigo-600 text-white shadow-sm' : 'text-slate-600 hover:text-slate-800'"
      ...
      <div class="canvas-stage-wrapper bg-slate-50 p-4 rounded-2xl border border-slate-200 overflow-x-auto">
        <div
    ...
    <div class="w-full lg:w-80 bg-white border border-slate-200 rounded-2xl p-5 shadow-sm flex flex-col gap-6 flex-shrink-0">
      ...
      <div class="warnings-section">
        <h3 class="text-xs font-bold uppercase tracking-wider text-slate-400 mb-3 flex justify-between items-center">
          ...
          <span class="warning-count bg-amber-100 text-amber-800 text-xxs font-bold px-2 py-0.5 rounded-full">
            {{ layoutWarnings.length }}
          </span>
        </h3>
        
        <div v-if="layoutWarnings.length === 0" class="text-xs text-emerald-600 bg-emerald-50 border border-emerald-100 rounded-lg p-3 flex items-center gap-2">
        ...
        <div v-else class="space-y-2 max-h-[220px] overflow-y-auto pr-1">
          ...
          >
            <div class="flex items-center justify-between font-semibold text-xs text-amber-800 mb-0.5">
              <span class="truncate max-w-[150px]">{{ warning.label }}</span>
              <span class="text-[10px] bg-amber-100 px-1 py-0.2 rounded font-mono">Span: {{ warning.gridSpan }}</span>
            </div>
      ...
      <div class="selected-field-section border-t border-slate-100 pt-5">
        ...
        <div v-if="!selectedField" class="text-xs text-slate-500 italic bg-slate-50 border border-slate-100 rounded-lg p-3">
          Select a field widget on the canvas to inspect and edit its layout attributes.
        </div>
        
        <div v-else class="space-y-4 bg-slate-50 border border-slate-100 rounded-xl p-4">
          <div class="flex items-center justify-between text-xs font-semibold text-slate-700 border-b border-slate-200 pb-2">
            <span class="truncate max-w-[120px]">{{ selectedField.label }}</span>
            <span class="font-mono text-xxs bg-slate-200 px-1.5 py-0.5 rounded text-slate-600">{{ selectedField.id }}</span>
          </div>
          ...
          <div class="form-group flex flex-col gap-1.5">
            ...
            >
              <option v-for="n in 12" :key="n" :value="n">
                {{ n }} {{ n === 12 ? 'Columns (Full Width)' : n === 6 ? 'Columns (Half Width)' : 'Columns' }}
              </option>
            </select>
          </div>
          ...
          <div class="flex items-center gap-2">
            ...
            />
            <label for="inspect-field-required" class="text-xs font-medium text-slate-700">Required Field</label>
          </div>
        </div>
      </div>
      ...
      <div class="compiler-section border-t border-slate-100 pt-5">
        ...
        <div class="space-y-3 bg-slate-50 border border-slate-100 rounded-xl p-4">
          <div class="flex items-center gap-2">
            <input
              ...
              class="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500"
            />
            <label for="dismiss-warnings-checkbox" class="text-xs font-medium text-slate-700">
          ...
          <div v-if="compilationStatus === 'blocked'" class="compilation-error text-xs text-red-700 bg-red-50 border border-red-100 rounded-lg p-2.5 flex items-start gap-1.5 font-medium leading-snug">
            <span class="flex-shrink-0">❌</span>
            <span>Compilation blocked: active layout warnings must be resolved or explicitly dismissed!</span>
          </div>
          <div v-if="compilationStatus === 'success'" class="compilation-success text-xs text-emerald-700 bg-emerald-50 border border-emerald-100 rounded-lg p-2.5 flex items-start gap-1.5 font-medium leading-snug">
            <span class="flex-shrink-0">🎉</span>
            <span>Compilation successful! Form schema successfully translated.</span>
          </div>
        </div>
      </div>
    </div>
