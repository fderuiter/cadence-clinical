        ...
        <tbody>
          ...
          >
            <td class="cell-type">
              <span
                class="type-badge"
                :class="item.criterion_type.toLowerCase()"
              >
                {{ item.criterion_type }}
              </span>
            </td>
...
const inclusionCount = computed(() => {
  return localCriteria.value.filter((c) => c.criterion_type === "INCLUSION")
    .length;
});

const exclusionCount = computed(() => {
  return localCriteria.value.filter((c) => c.criterion_type === "EXCLUSION")
    .length;
});
...
const addCriterion = () => {
  ...
  const defaultId =
    newCritId.value.trim() ||
    `${newCritType.value === "INCLUSION" ? "INC" : "EXC"}-${localCriteria.value.length + 1}`;
