...
describe("Global Library Management UI & Governance", () => {
  ...
  beforeEach(() => {
    ...
    window.addLedgerBlock = vi.fn().mockImplementation((action, details, reason) => {
      ledgerBlocks.push({ action, details, reason, timestamp: new Date().toISOString() });
    ...
    window.GlobalLibrarySandbox = {
      mockLibraryObjects: [
        {
          ...
          history: [
            { version: "1.0.0", status: "PUBLISHED", change_reason: "Initial publication of demographics form.", updated_by: "usr_dm_fderuiter", updated_at: "2026-08-01T12:00:00Z" }
          ]
        },
        {
          id: "lib-arm-placebo",
          ...
          history: [
            { version: "1.0.0", status: "DRAFT", change_reason: "Draft version.", updated_by: "usr_designer_alice", updated_at: "2026-08-03T11:00:00Z" }
          ]
        },
        {
          ...
          history: [
            { version: "1.0.0", status: "PUBLISHED", change_reason: "Published visit.", updated_by: "usr_dm_fderuiter", updated_at: "2026-08-04T09:15:00Z" }
          ]
        }
      ],
      ...
      renderLibraryDetails() {
        ...
        const obj = this.mockLibraryObjects.find(o => o.id === this.selectedLibraryObjectId);
        if (!obj) {
      ...
      handleTransitionLibraryConfirm(id, targetStatus, reason, role) {
        ...
        if (!allowedRoles.includes(role)) {
          this.displayLibraryError(`Authorization Failure: Role '${role}' is not authorized.`);
          return;
        }
        ...
        if (obj) {
          ...
          obj.version = "1.0.1";
          obj.history.unshift({ version: "1.0.1", status: targetStatus, change_reason: reason });
          this.clearLibraryError();
          this.renderLibrary();
          this.renderLibraryDetails();
          window.addLedgerBlock("LIBRARY_TRANSITION", { id, targetStatus }, reason);
        }
      },

      handleInstantiateLibraryConfirm(id, targetStudyId, reason, role) { // eslint-disable-line no-unused-vars
        const obj = this.mockLibraryObjects.find(o => o.id === id);
        if (!obj) return;
        ...
        if (obj.object_type === "FORM") {
          window.currentUsdm.forms.push({ name: obj.id, statuses: ["Pending"] });
        } else if (obj.object_type === "ARM") {
          window.currentUsdm.arms.push({ arm_id: obj.id, arm_name: `Arm ${obj.id}` });
        } else if (obj.object_type === "VISIT") {
          window.currentUsdm.encounters.push({ encounter_id: obj.id, encounter_name: `Visit ${obj.id}` });
        }

        window.renderMdr();
        this.clearLibraryError();
        window.addLedgerBlock("LIBRARY_INSTANTIATE", { id, targetStudyId }, reason);
      }
    };
  });
  ...
  it("displays payload details, lifecycle status, and version histories in the details card", () => {
    ...
    expect(document.getElementById("library-details-content").textContent).toContain("Select a library object");
    ...
    expect(detailsContent.querySelector(".status-badge").textContent).toBe("PUBLISHED");
    expect(detailsContent.querySelector(".payload-box").textContent).toContain("FORM");
    expect(detailsContent.querySelector(".history-table").textContent).toContain("v1.0.0 - PUBLISHED");
  });
  ...
  it("verifies and transitions lifecycle status correctly on role validation checks", () => {
    ...
    sandbox.handleTransitionLibraryConfirm("lib-arm-placebo", "IN_REVIEW", "Request review justification", "unauthorized_role");

    let errorBanner = document.getElementById("library-error-banner");
    expect(errorBanner.style.display).toBe("block");
    expect(errorBanner.textContent).toContain("Authorization Failure: Role 'unauthorized_role' is not authorized");

    // Try to transition with an authorized role (sponsor_designer)
    sandbox.handleTransitionLibraryConfirm("lib-arm-placebo", "IN_REVIEW", "Transition review request.", "sponsor_designer");

    errorBanner = document.getElementById("library-error-banner");
    expect(errorBanner.style.display).toBe("none"); // cleared

    const placebo = sandbox.mockLibraryObjects.find(o => o.id === "lib-arm-placebo");
  ...
  it("instantiates FORM, ARM, and VISIT objects to update active study schemas", () => {
    ...
    // 1. Instantiate FORM
    sandbox.handleInstantiateLibraryConfirm("lib-form-demographics", "STUDY-USDM-001", "Instantiate demographics", "sponsor_dm");
    expect(mockUsdm.forms).toHaveLength(1);
    expect(mockUsdm.forms[0].name).toBe("lib-form-demographics");
    expect(mdrRendered).toBe(true);

    // 2. Instantiate ARM
    sandbox.handleInstantiateLibraryConfirm("lib-arm-placebo", "STUDY-USDM-001", "Instantiate arm placebo", "sponsor_dm");
    expect(mockUsdm.arms).toHaveLength(1);
    expect(mockUsdm.arms[0].arm_id).toBe("lib-arm-placebo");

    // 3. Instantiate VISIT
    sandbox.handleInstantiateLibraryConfirm("lib-visit-screening", "STUDY-USDM-001", "Instantiate screening visit", "sponsor_dm");
    ...
    sandbox.handleInstantiateLibraryConfirm("lib-form-demographics", "WRONG-STUDY-999", "Instantiate demographics", "sponsor_dm");
    const errorBanner = document.getElementById("library-error-banner");
    expect(errorBanner.style.display).toBe("block");
    expect(errorBanner.textContent).toContain("Validation Error: Incorrect Study ID");
