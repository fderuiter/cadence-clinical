...
describe("eConsent API Client Unit Tests", () => {
  ...
  it("should list templates and append query string correctly", async () => {
    ...
    const res = await econsentService.listTemplates({ study_id: "study-01", all_versions: true });
    expect(res).toEqual([{ template_id: "tpl-1" }]);
    expect(mockFetch).toHaveBeenCalledTimes(1);

    const [url, options] = mockFetch.mock.calls[0];
    expect(url).toBe("http://localhost:8000/api/v1/econsent/templates?study_id=study-01&all_versions=true");
  ...
  it("should create template with body and custom X-Change-Reason header on mutation", async () => {
    ...
    const body = { study_id: "study-01", template_name: "ICF", protocol_version: "v1.0" };
    const res = await econsentService.createTemplate(body, { changeReason: "Initial creation" });
    expect(res).toEqual({ template_id: "tpl-1" });
  ...
  it("should update template with body and custom X-Change-Reason", async () => {
    ...
    const res = await econsentService.updateTemplate("tpl-1", body, { changeReason: "Minor edit" });
  ...
  it("should get template with version_index query", async () => {
    ...
    const res = await econsentService.getTemplate("tpl-1", { version_index: 1 });
    expect(res).toEqual({ template_id: "tpl-1", version_index: 1 });

    const [url, options] = mockFetch.mock.calls[0];
    expect(url).toBe("http://localhost:8000/api/v1/econsent/templates/tpl-1?version_index=1");
    expect(options.method).toBe("GET");
  });
  ...
  it("should compose template with version_index query", async () => {
    ...
    const res = await econsentService.composeTemplate("tpl-1", { version_index: 2 });
    expect(res).toEqual({ template_id: "tpl-1" });

    const [url, options] = mockFetch.mock.calls[0];
    expect(url).toBe("http://localhost:8000/api/v1/econsent/templates/tpl-1/compose?version_index=2");
    expect(options.method).toBe("GET");
  });
  ...
  it("should publish template with POST and changeReason", async () => {
    ...
    const res = await econsentService.publishTemplate("tpl-1", { changeReason: "Publishing ICF" });
    expect(res).toEqual({ template_id: "tpl-1", is_published: true });

    const [url, options] = mockFetch.mock.calls[0];
    expect(url).toBe("http://localhost:8000/api/v1/econsent/templates/tpl-1/publish");
    expect(options.method).toBe("POST");
  ...
  it("should create clause and pass changeReason", async () => {
    ...
    const res = await econsentService.createClause({ title: "C1" }, { changeReason: "Initial clause" });
  ...
  it("should update clause", async () => {
    ...
    const res = await econsentService.updateClause("c-1", { title: "C1 updated" }, { changeReason: "Clause edit" });
    expect(res).toEqual({ clause_id: "c-1" });
  ...
  it("should list clauses", async () => {
    ...
    expect(url).toBe("http://localhost:8000/api/v1/econsent/clauses?study_id=study-01");
  });
  ...
  it("should get clause", async () => {
    ...
    const [url] = mockFetch.mock.calls[0];
    expect(url).toBe("http://localhost:8000/api/v1/econsent/clauses/c-1?version_index=3");
  });
  ...
  it("should define comprehension check", async () => {
    ...
    await econsentService.defineComprehensionCheck("tpl-1", 1, { questions: [] }, { changeReason: "Defining checks" });
    const [url, options] = mockFetch.mock.calls[0];
    expect(url).toBe("http://localhost:8000/api/v1/econsent/templates/tpl-1/versions/1/comprehension-checks");
    expect(options.method).toBe("POST");
  ...
  it("should handle translations endpoints", async () => {
    ...
    await econsentService.createTranslation({ source_id: "s-1" }, { changeReason: "Add trans" });
    expect(mockFetch).toHaveBeenLastCalledWith(
      "http://localhost:8000/api/v1/econsent/translations",
      expect.objectContaining({ method: "POST" })
    );

    await econsentService.updateTranslation("t-1", { source_id: "s-1" }, { changeReason: "Edit trans" });
    expect(mockFetch).toHaveBeenLastCalledWith(
      "http://localhost:8000/api/v1/econsent/translations/t-1",
      expect.objectContaining({ method: "PUT" })
    );

    await econsentService.listTranslations({ source_id: "s-1", language_code: "es" });
    expect(mockFetch).toHaveBeenLastCalledWith(
    ...
    await econsentService.transitionTranslation("t-1", { status: "APPROVED" }, { changeReason: "Approve trans" });
    expect(mockFetch).toHaveBeenLastCalledWith(
  ...
  it("should get approved composed content for participant", async () => {
    ...
    await econsentService.getApprovedContent("tpl-1", { language_code: "nl", version_index: 1 });
    const [url, options] = mockFetch.mock.calls[0];
    expect(url).toBe("http://localhost:8000/api/v1/econsent/templates/tpl-1/approved-content?language_code=nl&version_index=1");
    expect(options.method).toBe("GET");
  });
});
