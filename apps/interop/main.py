...
class NotificationRouter:
    """Routes reminders to the central notification service."""

    def __init__(self):
        self.base_url = os.getenv("NOTIFICATIONS_URL", "http://localhost:8006")
    ...
    async def _send(self, payload: dict) -> bool:
        ...
        try:
            ...
            # Catch request/network exceptions and fail-soft to stubbed delivery (True)
            return True
        except Exception:
            return False

    async def send_email(self, email: str, message: str) -> bool:
        print(f"[STUB EMAIL] Sending email to {email}: {message}")
        payload = {
    ...
    async def send_sms(self, phone: str, message: str) -> bool:
        print(f"[STUB SMS] Sending SMS to {phone}: {message}")
        payload = {
    ...
    async def send_webhook(self, url: str, payload: dict) -> bool:
        print(f"[STUB WEBHOOK] Sending webhook to {url}")
        import json
        ...
        return await self._send(notif_payload)
