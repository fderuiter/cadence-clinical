...
def validate_epro_payload(answers: dict[str, Any]) -> list[str]:
    """Validate the ePRO/eCOA answers dict for correctness."""
    errors = []
    if "age" in answers:
        try:
            age = int(answers["age"])
...
async def resolve_and_save_submission(
    ...
    stmt = select(EPROSubmission).where(
        EPROSubmission.subject_id == payload.subject_id,
        EPROSubmission.diary_id == payload.diary_id,
    )
    ...
    if status_res == "CREATED":
        ...
        return {
            ...
            "audit_details": {"action": "EPRO_RECONCILE", "details": audit_details},
        }

    if status_res == "UPDATED_CLIENT_WINS":
        defeated_sub = EPROSubmissionDefeated(
    ...
    if status_res == "IGNORED_SERVER_WINS":
        defeated_sub = EPROSubmissionDefeated(
            subject_id=payload.subject_id,
            diary_id=payload.diary_id,
            device_timestamp=payload.device_timestamp,
...
async def fhir_prefill(
    ...
    """Ingest a standard FHIR Bundle payload and return mapped CDASH eCRF fields."""
...
@router.post("/api/v1/interop/fhir/pre-screen", response_model=FHIRPreScreenResponse)
async def fhir_pre_screen(
    request: Request,
    payload: FHIRPreScreenRequest,
    session: AsyncSession = Depends(get_db_session),
) -> FHIRPreScreenResponse:
    """Advisory FHIR eligibility pre-screening endpoint."""
    require_staff_role(request)
    user_id = getattr(request.state, "user_id", "system")
    user_roles = getattr(request.state, "roles", "system")
    change_reason = getattr(request.state, "change_reason", "system_operation")
    ...
    from apps.interop import main as main_module

    criteria = await main_module.fetch_eligibility_criteria(payload.study_id)
...
@router.post("/api/v1/interop/epro/submit", status_code=201)
async def epro_submit(
    request: Request,
    payload: EPROSubmissionPayload,
    session: AsyncSession = Depends(get_db_session),
) -> dict[str, Any]:
    """Submit a single participant diary/survey entry."""
...
@router.post("/api/v1/interop/epro/sync", status_code=200)
async def epro_sync(
    request: Request,
    ...
    for sub_payload in payload.submissions:
        ...
        if st == "CREATED":
            created_count += 1
        elif st in ("UPDATED_CLIENT_WINS", "MERGED"):
            updated_count += 1
        elif st == "IGNORED_SERVER_WINS":
            ignored_count += 1
        elif st == "STRUCTURAL_CONFLICT":
            conflict_count += 1
        elif st == "QUARANTINED":
            quarantine_count += 1
...
@router.get("/api/v1/interop/instruments/{id}", response_model=InstrumentResponse)
...
async def deliver_notification_task(
    ...
    async with session_maker() as session:
        try:
            ...
            if channel == "EMAIL":
                success = await notif_router.send_email(
                    f"{subject_id}@example.com", message
                )
            elif channel == "SMS":
                success = await notif_router.send_sms(
                    "+1234567890", message
                )  # deid-ignore
                ...
                success = await notif_router.send_webhook(
                    f"https://hooks.example.com/subject/{subject_id}",  # deid-ignore
                    webhook_payload,
                )
            elif channel == "IN_APP":
                success = await notif_router.send_in_app(subject_id, message)
            else:
                success = False
...
@router.post("/api/v1/interop/reminders/compute", status_code=200)
...
def require_trial_manager_role(request: Request) -> None:
    """Ensure the requester is a clinical trial manager or administrator."""
...
async def edit_quarantined_submission(
    ...
    """Edit a quarantined ePRO submission payload."""
    require_trial_manager_role(request)
    user_id = getattr(request.state, "user_id", "system")
...
async def replay_quarantined_submission(
    ...
    defeated_sub = EPROSubmissionDefeated(
        ...
        diary_id=entry.diary_id,
        device_timestamp=entry.device_timestamp,
        answers=entry.original_answers,
