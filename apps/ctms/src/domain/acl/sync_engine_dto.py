"""Anti-Corruption Layer DTOs and reconciliation logic for Offline Sync in CTMS Service."""
...
class CTMSSignatureValidationError(ValueError):
    """Exception raised when signature validation fails for CTMS sync records."""

    pass


class CTMSSyncMetadataDTO(BaseModel):
    """Conflict resolution and validation metadata for CTMS offline sync records."""
...
class CTMSSyncRecordDTO(BaseModel):
    """Local CTMS ACL DTO representing a synchronization record payload."""
...
class CTMSSyncReconciliationResultDTO(BaseModel):
    """Result of CTMS offline record reconciliation."""
...
def normalize_to_utc(dt: datetime) -> datetime:
    """Normalize a datetime object to timezone-aware UTC."""
...
def get_ctms_signature_payload(record: CTMSSyncRecordDTO) -> dict[str, Any]:
    """Construct canonical JSON payload dictionary for HMAC signature verification."""
    timestamps_dict = {}
...
def verify_ctms_record_signature(record: CTMSSyncRecordDTO, secret: bytes) -> bool:
    """Verify HMAC-SHA256 signature against canonically serialized payload."""
...
def reconcile_ctms_records(
    ...
    """Reconcile existing CTMS record data and incoming CTMSSyncRecordDTO according to conflict strategy."""
    ...
    # MERGE Strategy via Last-Write-Wins (LWW)
    merged_data = {}
