...
async def test_monitoring_visit_scheduling_respects_cra_allocation():
    ...
    visit_date = datetime.now(UTC) + timedelta(days=5)
    visit_payload = {
        "study_id": "study_Y",
        "site_id": "site_B",
        "cra_id": "cra_charlie",
...
async def test_ctms_sync_happy_path_and_reloads():
    ...
    actual_date = datetime.now(UTC) - timedelta(hours=1)
    sync_payload = {
        "visit_id": visit_id,
        "actual_date": actual_date.isoformat(),
        "findings": [
...
async def test_ctms_sync_conflict_server_wins():
    ...
    actual_date_server = datetime.now(UTC) - timedelta(hours=5)
    client.post(
    ...
    actual_date_client = datetime.now(UTC) - timedelta(hours=1)
    sync_payload = {
        "visit_id": visit_id,
...
async def test_ctms_sync_conflict_merge():
    ...
    # First completion
    date_initial = datetime.now(UTC) - timedelta(hours=10)
    client.post(
        "/api/v1/ctms/monitoring-visits/sync",
        json={
            "visit_id": visit_id,
    ...
    date_newer = datetime.now(UTC) - timedelta(hours=1)
    payload_merge = {
        "visit_id": visit_id,
