...
async def execute_ctms_audit_sealing_cycle(
    ...
    last_block_query = await db.execute(
        text(
            "SELECT current_block_hash FROM ctms_audit_ledger_seals ORDER BY block_index DESC LIMIT 1;"
        )
    )
    ...
    unsealed_query = await db.execute(
        text(
            "SELECT id, timestamp, user_id, user_role, action, details "
            "FROM ctms_audit_logs WHERE cryptographic_seal IS NULL ORDER BY timestamp ASC, id ASC LIMIT :limit;"
        ),
        {"limit": limit},
    )
    ...
    await db.execute(
        text(
            "INSERT INTO ctms_audit_ledger_seals (previous_block_hash, current_block_hash, timestamp, sealed_record_count, merkle_root_hash) "
    ...
    for rec_id in record_ids:
        await db.execute(
            text(
                "UPDATE ctms_audit_logs SET cryptographic_seal = :seal WHERE id = :id;"
            ),
    ...
    try:
        ...
        seals_query = await db.execute(
            text(
                ...
                "FROM ctms_audit_ledger_seals ORDER BY block_index ASC;"
            )
        )
        ...
        for seal in seals:
            ...
            records_query = await db.execute(
                text(
                    "SELECT id, timestamp, user_id, user_role, action, details "
                    "FROM ctms_audit_logs WHERE cryptographic_seal = :seal ORDER BY timestamp ASC, id ASC;"
                ),
        ...
        sealed_records_query = await db.execute(
            text(
                "SELECT DISTINCT cryptographic_seal FROM ctms_audit_logs WHERE cryptographic_seal IS NOT NULL;"
            )
        )
        ...
        if orphan_seals:
            raise ValueError(f"Found orphan seals on CTMS audit logs: {orphan_seals}")

        return True
    except Exception as e:
        TrialLockManager.lock_trial(reason=f"CTMS GxP Data Integrity Breach: {str(e)}")
        raise ValueError(f"CTMS GxP Data Integrity Breach: {str(e)}") from e
