...
from apps.ctms.models import CTMSAuditLog, CTMSDelegation
...
class SQLAlchemCTMSDelegationRepository(ICTMSDelegationRepository):
    def __init__(self, session: AsyncSession):
        self.session = session

    @map_database_exceptions
    async def get_by_id(self, record_id: str) -> CTMSDelegationEntity | None:
        stmt = select(CTMSDelegation).where(CTMSDelegation.id == record_id)
...
async def get_ctms_repository(
    ...
    return SQLAlchemCTMSDelegationRepository(session)
