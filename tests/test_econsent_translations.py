...
async def test_translation_crud_and_validation():
    ...
    res = client.post(
        "/api/v1/econsent/translations", json=trans_payload, headers=headers
    )
    ...
    res = client.put(
        f"/api/v1/econsent/translations/{translation_id}",
        json=update_payload,
        headers=headers,
    )
...
async def test_approved_content_retrieval_and_cache():
    ...
    res = client.get(
        "/api/v1/econsent/templates/t1/approved-content?language_code=es",
        headers=headers,
    )
    ...
    for tid in (t_trans_id, c_trans_id):
        client.post(
            f"/api/v1/econsent/translations/{tid}/transition",
            json={"status": "IN_REVIEW", "reason_for_change": "Reviewing"},
            headers=headers,
        )
        client.post(
            f"/api/v1/econsent/translations/{tid}/transition",
    ...
    with patch.object(approved_translation_cache, "ttl", 0.05):
        ...
        with patch(
            "apps.econsent.main.fetch_composed_translation_from_db"
        ) as mock_fetch:
    ...
    with patch.object(approved_translation_cache, "ttl", 0.01):
        ...
        with patch(
            ...
            assert (
                res.status_code == 200
            )  # Returns the stale/expired entry successfully!
    ...
    res = client.put(
        f"/api/v1/econsent/translations/{c_trans_id}",
        json={
            "source_id": "c1",
            "source_type": "clause",
            "source_version_index": 1,
    ...
    client.post(
        f"/api/v1/econsent/translations/{c_trans_id}/transition",
        json={"status": "IN_REVIEW", "reason_for_change": "Reviewing new version"},
        headers=headers,
    )
    ...
    client.post(
        f"/api/v1/econsent/translations/{c_trans_id}/transition",
        json={"status": "APPROVED", "reason_for_change": "Approving new version"},
        headers=headers,
    )
