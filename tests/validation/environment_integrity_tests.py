...
def test_environment_integrity():
    ...
    assert sys.version_info >= (3, 11), f"Python version {sys.version} is less than 3.11!"
    ...
    for d in expected_dirs:
        ...
        assert dir_path.is_dir(), f"Core GxP directory boundary '{d}' is missing at {dir_path}!"

    # 3. Verify Presence of Critical Dependency Manifests
    assert (repo_root / "pyproject.toml").is_file(), "pyproject.toml manifest is missing!"
