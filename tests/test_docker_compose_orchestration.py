...
def test_docker_compose_named_volumes():
    ...
    sqlite_services = [
        "eisf", "etmf", "ctms", "quality", "interop", "tickets", "safety", "notifications", "econsent"
    ]
    ...
    for service_name in sqlite_services:
        ...
        assert has_named_volume, f"Service {service_name} is missing a mount for named volume {volume_name}"
        ...
        assert db_url_env_var is not None, f"Service {service_name} is missing database URL environment variable"
        assert f"/app/{service_name}_data/" in db_url_env_var, f"Database URL {db_url_env_var} for {service_name} does not point to the named volume mount directory /app/{service_name}_data/"
    ...
    expected_root_volumes = [f"{s}_data" for s in sqlite_services] + ["postgres_data", "neo4j_data"]
...
def test_docker_compose_targeted_reloads():
    ...
    services_with_reloads = [
        "designer", "execution", "org", "eisf", "etmf", "ctms", "quality", "interop", "tickets", "safety", "notifications", "econsent", "gateway"
    ]
    ...
    for service_name in services_with_reloads:
        ...
        assert "--reload" in command_str, f"Service {service_name} command does not have --reload option"
        assert "--reload-dir" in command_str, f"Service {service_name} command does not have --reload-dir option"
        assert f"/app/apps/{service_name}" in command_str, f"Service {service_name} command is not watching /app/apps/{service_name}"
...
def test_docker_compose_node_modules_isolation():
    ...
    assert "/app/node_modules" in volumes, "subject-portal service is missing anonymous volume mount for /app/node_modules"
    assert "/app/apps/subject-portal/node_modules" in volumes, "subject-portal service is missing anonymous volume mount for /app/apps/subject-portal/node_modules"
...
def test_docker_compose_user_permissions_mapped():
    ...
    custom_services = [
        "designer", "execution", "org", "eisf", "etmf", "ctms", "quality", "interop", "tickets", "safety", "notifications", "econsent", "subject-portal", "gateway"
    ]

    for service_name in custom_services:
        service = config["services"][service_name]
        user_mapping = service.get("user")
        assert user_mapping == "${UID:-1000}:${GID:-1000}", f"Service {service_name} does not have UID:GID user mapping configured"
