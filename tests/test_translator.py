...
@pytest.mark.asyncio
async def test_phase13_xpath_compilation_and_extensions():
    ...
    assert extract_constraint_expression(item_with_rules) == "(/heart_rate < 200)"

    # Integration level verification via published event
    study_payload = {
        "study_id": "phase13_test_study",
        "payload": {
            "name": "Phase 13 Test Trial",
            "protocol": {
                "items": [
                    {"id": "sys_bp", "name": "Systolic Blood Pressure", "type": "int"},
                    item_with_rules,
    ...
    for _ in range(50):
        async with db_manager.get_session_maker()() as session:
            result = await session.execute(
                TranslationJob.__table__.select().where(
                    TranslationJob.study_id == "phase13_test_study"
                )
            )
            job = result.mappings().first()
            if job and job["status"] in ("COMPLETED", "FAILED"):
    ...
    rel_alias = [a for a in aliases if a.get("Context") == "relevant"][0]
    assert rel_alias.get("Name") == "(/sys_bp = 120)"

    con_alias = [a for a in aliases if a.get("Context") == "constraint"][0]
    assert con_alias.get("Name") == "(/heart_rate < 200)"

    # Verify existing ID-set-parity assertions are unbroken
    odm_ids = [item.get("OID") for item in item_defs]
    binds = model.findall("xf:bind", ns)
    openrosa_ids = [bind.get("nodeset").replace("/", "") for bind in binds]
    assert set(odm_ids) == set(openrosa_ids)
