...
async def test_epro_submission_part11_fields_retention():
    ...
    client = TestClient(app)
    headers = get_auth_headers(
        roles="Subject", change_reason="Initial ePRO submission by Subject", user_id="subj_part11"
    )
    ...
    update_headers = get_auth_headers(
        roles="Subject", change_reason="Corrected value after correction", user_id="subj_part11"
    )
    sub_payload["answers"]["value"] = 15
    sub_payload["offline_sync_markers"]["sequence_number"] = 2

    resp2 = client.post("/api/v1/interop/epro/submit", json=sub_payload, headers=update_headers)
