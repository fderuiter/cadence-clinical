...
from tests.rbac_helpers import data_manager, build_gateway_headers, sponsor_designer
...
async def test_rbac_execution_unauthorized(shared_sqlite_dbs, execution_client):
    ...
    """
    headers = build_gateway_headers(user_id="test_subject", roles="subject", change_reason="Trying to write")
    payload = {
        "study_id": "study_1",
        "subject_id": "sub_1",
        "visit_id": "visit_1",
    ...
    resp = await execution_client.post("/api/v1/execution/queries", json=payload, headers=headers)
...
async def test_rbac_etmf_site_scoping(shared_sqlite_dbs, etmf_client, signed_headers):
    ...
    """
    admin_hdrs = signed_headers(user_id="admin_user", roles="admin", change_reason="Setup documents")
    ...
    resp_alpha = await etmf_client.get(f"/api/v1/etmf/documents/{doc_alpha_id}", headers=alpha_hdrs)
    assert resp_alpha.status_code == 200

    # 2. View site_beta doc -> should fail with 403
    resp_beta = await etmf_client.get(f"/api/v1/etmf/documents/{doc_beta_id}", headers=alpha_hdrs)
...
async def test_rbac_designer_study_scoping(designer_client, mock_designer_driver):
    ...
    with override_principal(designer_app, principal):
        ...
        resp_deny = await designer_client.get("/api/v1/studies/study_foo/export", headers=headers)
        ...
        resp_allow = await designer_client.get("/api/v1/studies/study_bar/export", headers=headers)
