...
@pytest.mark.asyncio
async def test_etmf_qc_lifecycle_and_audit():
    """
    Verify eTMF multi-stage QC lifecycle and its Part 11 audit obligations.
    Covers:
    - Successful progression: Draft -> Technical QC -> Clinical QC -> Approved -> Archived.
    - Rejection from applicable active stages and return from REJECTED to DRAFT.
    - Rejection of invalid/reverse transitions.
    - Role-gate failures returning HTTP 403.
    - Missing or empty change-reason failures.
    - Persistence of a DocumentQCTransition and a TMFAuditLog entry for every successful transition.
    ...
    payload1 = {
        "study_id": "study_happy",
        "artifact_type": "Clinical Trial Protocol",
        "filename": "protocol_happy.pdf",
    ...
    # CLINICAL_QC -> APPROVED (Allowed roles: sponsor_dm, sponsor_clinical, admin)
    resp = client.post(
        f"/api/v1/etmf/documents/{doc_id1}/transition",
        json={"to_status": "APPROVED", "reason_for_change": "All approvals retrieved"},
    ...
    # 2. Rejection from applicable active stages and return from REJECTED to DRAFT.
    payload2 = {
        "study_id": "study_reject",
        "artifact_type": "Clinical Trial Protocol",
        "filename": "protocol_reject.pdf",
    ...
    # Try TECHNICAL_QC -> ARCHIVED (disallowed)
    ...
    # 4. Role-gate failures returning HTTP 403.
    # From TECHNICAL_QC, try to transition to CLINICAL_QC with a role that is NOT allowed.
    ...
    # 5. Missing or empty change-reason failures.
    resp = client.post(
        f"/api/v1/etmf/documents/{doc_id2}/transition",
        json={"to_status": "CLINICAL_QC", "reason_for_change": "Short"},
        headers=headers_clinical,
    )
    assert resp.status_code == 422
    ...
    resp = client.post(
        f"/api/v1/etmf/documents/{doc_id2}/transition",
        json={"to_status": "CLINICAL_QC", "reason_for_change": "          "},
        headers=headers_clinical,
    )
    assert resp.status_code == 422
    assert (
    ...
    resp_hist = client.get(
        f"/api/v1/etmf/documents/{doc_id1}/qc-history",
        headers=headers_admin,
    )
...
async def test_etmf_audit_logs_filtering_and_pagination():
    ...
    """
    client = TestClient(app)
    admin_headers = get_auth_headers(roles="admin", change_reason="Filtering and pagination testing setup")
    inspector_headers = get_auth_headers(roles="regulatory_inspector")
    ...
    doc_id_b = ingest_b.json()["document_id"]

    # 3. Retrieve audit-logs with pagination and check descending ordering
    resp = client.get("/api/v1/etmf/audit-logs?limit=3&offset=0", headers=inspector_headers)
    assert resp.status_code == 200
    data = resp.json()
    ...
    resp_user = client.get("/api/v1/etmf/audit-logs?user_id=test_user", headers=inspector_headers)
    ...
    resp_action = client.get("/api/v1/etmf/audit-logs?action=INGEST", headers=inspector_headers)
    ...
    resp_doc_a = client.get(f"/api/v1/etmf/audit-logs?document_id={doc_id_a}", headers=inspector_headers)
    ...
    sample_timestamp = datetime.fromisoformat(sample_timestamp_str.replace("Z", "+00:00"))
    ...
    if total_count > 1:
        ...
        resp_paginated = client.get("/api/v1/etmf/audit-logs?limit=1", headers=inspector_headers)
