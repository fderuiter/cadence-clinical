...
def test_assert_secure_secrets_validation(monkeypatch):
    ...
    with pytest.raises(RuntimeError) as exc_info:
        assert_secure_secrets("test-service", {"GATEWAY_SECRET": "internal-gateway-secret-12345"})
...
def test_global_scanner_with_opt_out(tmp_path):
    ...
    file_a.write_text("GATEWAY_SECRET = os.getenv('GATEWAY_SECRET', 'internal-gateway-secret-12345')", encoding="utf-8")
    ...
    file_b.write_text("GATEWAY_SECRET = os.getenv('GATEWAY_SECRET', 'internal-gateway-secret-12345')", encoding="utf-8")
