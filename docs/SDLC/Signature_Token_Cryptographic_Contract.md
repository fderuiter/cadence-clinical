...
5. **Action/Endpoint Binding:** Retrieve `action` from the token. Verify that the incoming request path matches the bound action (or is symmetrically subsetted). This prevents a token generated for form "A" approval from being maliciously re-routed to approve form "B" or authorize unblinding.
