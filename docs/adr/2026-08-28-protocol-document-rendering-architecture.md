# ADR-118: Protocol Document Rendering Architecture
...
We need a net-new capability to generate clinical trial protocol documents (comprising narrative content, synopsis, and the Schedule of Activities (SoA) matrix) programmatically. Prior to this, there was no existing PDF/DOCX infrastructure in the codebase.
The document must be compiled and rendered into standard, human-readable formats such as PDF and DOCX directly from digital clinical study definitions conforming to CDISC USDM standards.
...
Establishing a clear architectural boundary is required to define:
- Which service owns the templates and rendering orchestration.
- The server-side rendering library stack for PDF and DOCX.
- The threading boundaries for CPU-bound rendering tasks in asynchronous frameworks.
- A standard, layout-agnostic, render-oriented data contract (presentation-centric view models) in `core-models` that cleanly bridges raw USDM data structures and presentation templates.

This decision relates to adjacent SoA and rendering context documented in [2026-07-22-metadata-driven-grid-layouts.md](2026-07-22-metadata-driven-grid-layouts.md) and [2023-01-01-neo4j-graph-database.md](2023-01-01-neo4j-graph-database.md). This decision implements requirements under PRD-SYS-001.

## 2. Decision Drivers & Constraints
- **USDM-driven / Real-time Generation:** Protocol documents must be compiled directly from CDISC USDM study definitions in real-time.
- **Part 11 Auditability & GxP Compliance (PRD-SYS-001):** Exported protocol documents are regulatory records. Every export metadata record must enforce GxP-compliant fields: creator identity (`created_by`), UTC timestamp (`created_at`), change reason (`reason_for_change`), and a sequential version index (`version_index`).
- **Separation of Concerns:** Templates and document assembly logic must not be coupled directly to raw Neo4j graph schemas or the standard CDISC USDM schema. They must receive flat, pre-ordered presentation-centric data models.
- **Event-Loop Protection:** PDF generation (rendering HTML/CSS via a layout engine) and DOCX compilation are CPU-intensive, blocking operations. They must never block FastAPI's asynchronous event loop.
- **Layout Quality & Advanced Printing Standards:** The Schedule of Activities (SoA) matrix requires wide, landscape page formatting. Running headers, footers, exact page numbers, and table-of-contents page references are mandatory.
...
### Option 3: Server-Side Real-Time HTML-to-PDF/DOCX via Jinja2, WeasyPrint, and docxtpl (Selected)
Generate structured HTML from presentation models using Jinja2, compile PDFs using WeasyPrint (a Python-based visual layout engine with excellent support for CSS Paged Media), and compile DOCX files programmatically using `docxtpl` (Word-template to DOCX).
- **Pros:**
  ...
  - ✅ Reuses `docxtpl` for native-grade Microsoft Word files.
- **Cons:**
...
* **Justification:** Option 3 is the only approach that meets GxP/Part 11 standards (PRD-SYS-001) for deterministic rendering, provides advanced CSS Paged Media features for printing clinical SoAs, and maintains clean code structure via Jinja2 HTML templates and Word-templates with `docxtpl`.

### Architectural Specifications & Boundaries

#### A. Service Placement / Service Ownership and Boundary
...
#### B. Synchronous vs Asynchronous (Synchronous Generation Boundaries)
...
1. **Ordered Narrative Section Views:** NarrativeContent (with sectionNumber, sectionTitle, displaySectionNumber, displaySectionTitle, ordering/child references) and NarrativeContentItem (text).
...
4. **Export Metadata:** Explicit 21 CFR Part 11 audit details (`created_at`, `created_by`, `reason_for_change`, `version_index`) nested alongside every persisted or exported protocol document version.
...
* **Negative Impact / Technical Debt:**
  ...
  - ❌ Memory usage spikes during extremely large table rendering.
* **Mitigation Strategy:** Enforce pagination and chunking strategies if memory thresholds are exceeded.
...
* **Verification Plan:**
  ...
  2. Verify that standard build and dependency installations succeed.
