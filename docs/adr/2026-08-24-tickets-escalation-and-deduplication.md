...
1. **State Persistence:** Added `last_escalated_at`, `last_escalation_notified_at`, and `escalation_count` to the `Ticket` model in `apps/tickets/models/diff_models.py`.
...
* **Affected Files:** `apps/tickets/models/diff_models.py`, `apps/tickets/escalation.py`, `apps/tickets/main.py`
* **Verification Plan:**
