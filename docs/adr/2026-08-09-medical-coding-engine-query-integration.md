...
- **Justification:** Chosen Option 2 because atomic database transactions are critical for GxP data integrity. This guarantees that an uncodable verbatim term _always_ has its assignment set to `QUERY_PENDING` and its query raised in a single operation, with no risk of drift.
