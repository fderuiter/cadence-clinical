# ADR-251: Centralized Accessibility Matcher and Code Coverage Gates
...
* **WCAG 2.1 Compliance Enforcement:** Guardrails must ensure that both professional portal and patient portal widgets are strictly audited for screen-readers, contrast ratio, and layout.
...
* **Affected Repositories / Services:**
  * `packages/ui`: Centralized the `toBeAccessible` matcher and updated public entrypoints.
  * `apps/web`: Replaced local matcher setup with shared package imports.
  * `apps/subject-portal`: Standardized configuration to import `toBeAccessible` from `@cadence/ui`.
