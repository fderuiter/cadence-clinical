...
1. **Option A (Selected: Unified Multi-Channel Dispatch Engine with GxP Non-Opt-Out Matrix)**: Asynchronously dispatch via in-app polling drawer, SMTP with 21 CFR Part 11 headers, and HMAC-SHA256 signed webhooks. Mandatory non-opt-out matrix for safety-critical events (`CRITICAL` priority, SLA breaches).
2. **Option B (Synchronous Direct Transport)**: Trigger email and webhooks synchronously within core knowledge or ticketing transaction boundaries.
3. **Option C (Client-Side Direct Dispatch)**: Push notification routing logic to web client runtime.

## 4. Decision Outcome

Chosen option: **Option A (Unified Multi-Channel Dispatch Engine with GxP Non-Opt-Out Matrix)** because it provides fault-isolated, high-availability multi-channel notification dispatch while guaranteeing 21 CFR Part 11 audit trails, blinding/PII protection, and mandatory delivery for critical regulatory alerts.
