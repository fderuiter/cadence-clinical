...
## Acceptance Criteria
- [x] SoA Matrix editor occupies full viewport width with sticky encounter headers and activity row labels.
- [x] Toggling an activity-encounter intersection updates the backend study graph via REST API with optimistic UI updates.
- [x] Biomedical Concepts panel displays CDASH variable mappings (e.g. `VSORRES`, `SYSBP`, `DIABP`) with unit catalogs.
- [x] Inclusion & Exclusion criteria editor provides real-time syntactic check and severity tagging.
- [x] "Export USDM" button produces a schema-valid CDISC USDM v3.0/v4.0 JSON package matching `packages/usdm-schemas`.
- [x] Tests in `apps/designer/tests/` and `apps/web/tests/` pass with zero regressions.
