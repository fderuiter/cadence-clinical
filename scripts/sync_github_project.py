...
def assign_work_stream(i):
    ...
    if any(k in title or k in body for k in ['etmf', 'isf', 'document', 'archiving', 'tmf']):
        return "Stream 1: eTMF & Regulated Document Management", "Document Management & Archiving"
    elif any(k in title or k in body for k in ['rtsm', 'irt', 'dispensation', 'randomization', 'supply', 'blinded']):
        return "Stream 2: RTSM & IP Supply Chain", "RTSM / IRT v1.0"
    elif any(k in title or k in body for k in ['ecrf', 'soa', 'mdr', 'protocol', 'designer', 'form', 'crf']):
        return "Stream 3: Study Designer, eCRF & SoA", "Ultimate CRF Builder"
    elif any(k in title or k in body for k in ['ecoa', 'epro', 'subject portal', 'patient', 'econsent']):
        return "Stream 4: eCOA, ePRO & Subject Portal", "Native eCOA/ePRO Subject Portal v1.6.0"
    elif 'scope: frontend' in labels or any(k in title or k in body for k in ['spa', 'vue', 'ui', 'widget', 'component']):
        return "Stream 5: Frontend Vue 3 SPA", "Frontend SPA Completion"
    elif any(k in title or k in body for k in ['rbac', 'security', 'keycloak', 'audit', 'aes', 'crypto', 'auth', 'ticket']):
        return "Stream 6: Platform Security, RBAC & Audit Ledger", "Compliance & Security Controls"
    elif any(k in title or k in body for k in ['sdtm', 'adam', 'dataset', 'export', 'biostat', 'cdisc']):
        return "Stream 7: Biostatistics & Dataset Exports", "Biostatistical Export Pipeline v1.7.0 Completion"
    elif any(k in title or k in body for k in ['sdv', 'lab', 'monitoring', 'site', 'query', 'sae', 'reconciliation']):
...
def enhance_body_if_needed(i, parent_issues, blocked_by):
    ...
    if '🟢 **READY FOR DEV**' in body or '🔴 **BLOCKED**' in body or '🔵 **PARENT EPIC**' in body:
    ...
    if is_parent:
        ...
        readiness_note = f"Blocked by prerequisite issues: {', '.join(f'#{b}' for b in blocking_list)}" if blocking_list else "Blocked pending upstream prerequisite completion."
    else:
        status_badge = "🟢 **READY FOR DEV**"
        readiness_note = "Unblocked leaf task. Ready for immediate developer assignment."
        
    req_ids = sorted(list(set(re.findall(r'(PRD-[A-Z0-9-]+|Trace-\d+|ADR-\d+)', body))))
    req_str = ", ".join(req_ids) if req_ids else "PRD-SYS-001 (Core Platform Compliance)"
    
    file_targets = sorted(list(set(re.findall(r'`(apps/[^`]+|packages/[^`]+|docs/[^`]+|tests/[^`]+)`', body))))
...
def main():
    ...
    print("2. Checking for newly created issues requiring structure formatting...", flush=True)
    ...
    if missing_issues:
        print(f"Adding {len(missing_issues)} missing issues to Project Board...", flush=True)
    ...
    print(f"4. Synchronizing fields for {len(item_by_issue_num)} project items...", flush=True)
    for idx, (num, item) in enumerate(item_by_issue_num.items(), 1):
        ...
        if state == 'CLOSED':
            ...
            target_status = "Ready"
        elif '🔴 **blocked**' in body.lower() or 'blocked' in labels or '🔵 **parent epic**' in body.lower() or title.startswith('EPIC:'):
        ...
        if item.get('status') != target_status:
            run_cmd(["gh", "project", "item-edit", "--id", item_id, "--project-id", PROJECT_ID, "--field-id", STATUS_FIELD_ID, "--single-select-option-id", STATUS_OPTIONS[target_status]])
        if item.get('priority') != target_priority:
            run_cmd(["gh", "project", "item-edit", "--id", item_id, "--project-id", PROJECT_ID, "--field-id", PRIORITY_FIELD_ID, "--single-select-option-id", PRIORITY_OPTIONS[target_priority]])
        if item.get('size') != target_size:
            run_cmd(["gh", "project", "item-edit", "--id", item_id, "--project-id", PROJECT_ID, "--field-id", SIZE_FIELD_ID, "--single-select-option-id", SIZE_OPTIONS[target_size]])
