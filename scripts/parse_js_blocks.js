...
function getBabelParser() {
  ...
  if (fs.existsSync(pnpmDir)) {
    ...
    if (babelParserDir) {
      const parserPath = path.join(pnpmDir, babelParserDir, 'node_modules/@babel/parser/lib/index.js');
  ...
  try {
    ...
    const webNodeModules = '/app/apps/web/node_modules/@babel/parser/lib/index.js';
...
function getNodeKey(node) {
  ...
  if (node.type === 'ExportNamedDeclaration' && node.declaration) {
    ...
    if (decl.type === 'VariableDeclaration' && decl.declarations && decl.declarations[0] && decl.declarations[0].id) {
      return `var:${decl.declarations[0].id.name}`;
    }
  }
  if (node.type === 'VariableDeclaration' && node.declarations && node.declarations[0] && node.declarations[0].id) {
