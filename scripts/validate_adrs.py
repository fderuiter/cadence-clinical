...
def is_safe_arg(arg: str) -> bool:
    """Checks if an argument is safe from command/option injection."""
    if not arg:
        return True

    # Whitelist of allowed options/flags starting with hyphen
    allowed_options = {
        "-a",
...
def run_git_command(args: list[str]) -> tuple[str, str]:
    ...
    if not args or args[0] != "git":
        return "", "Invalid command: only git commands are allowed"

    for arg in args:
        if not is_safe_arg(arg):
            return "", f"Unsafe or unauthorized argument detected: {arg}"
...
def get_changed_files() -> set[str]:
    ...
    # 1. If in GitHub Actions, use origin/main diff as the primary source of truth
