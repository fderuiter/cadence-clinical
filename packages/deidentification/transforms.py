...
from packages.deidentification.detectors import resolve_overlaps
from packages.deidentification.models import DetectionResult, DetectorCategory

# Reconciled the date-shift window inconsistency into a single configurable parameter with a documented default
...
class RedactionRecordItem(BaseModel):
    ...
    strategy: str = Field(
        ...,
        description="The transform strategy applied (e.g., mask, full_mask, pseudonymize, date_shift, age_cap)",
    )
    start: int = Field(
...
def pseudonymize_value(value: str, salt: Union[str, bytes]) -> str:
    """
    Generates a deterministic, irreversible HMAC-SHA256 pseudonym for the given value.
    Reuses the approach established in apps/interop/fhir_adapter.py::pseudonymize_identifier.
...
def apply_deid_transforms(
    ...
    Args:
        ...
        strategies (Optional[dict]): Map of DetectorCategory to specific strategy ("mask", "full_mask", "pseudonymize", "date_shift", "age_cap").
