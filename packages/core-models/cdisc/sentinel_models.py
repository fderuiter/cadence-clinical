...
class QualityRuleFinding(BaseModel):
    ...
    category: str = Field(..., description="Category: Structural, Regulatory, Burden, Inconsistency")
...
class ReadabilityReport(BaseModel):
    ...
    flesch_kincaid_grade_level: float = Field(..., description="Flesch-Kincaid Grade Level")
    ...
    interpretation: str = Field(..., description="Human-readable readability description")


class BurdenTraceItem(BaseModel):
    """An itemized breakdown of clinical operational burden."""

    component: str = Field(..., description="The name of the component, e.g. visits, procedures, forms")
...
class BurdenTraceReport(BaseModel):
    ...
    procedure_burden: float = Field(..., description="Aggregated burden of clinical procedures")
    activity_burden: float = Field(..., description="Aggregated burden of CRFs and forms")
    total_burden: float = Field(..., description="Total clinical burden score")
    trace: List[BurdenTraceItem] = Field(default_factory=list, description="Trace details explaining the sum")
...
class AmendmentImpactReport(BaseModel):
    ...
    amended_version: Optional[str] = Field(None, description="Current amended version tag")
    ...
    estimated_cost_usd: float = Field(0.0, description="Estimated total cost in USD for this amendment")
    burden_change: float = Field(0.0, description="Burden index difference from base version")
    explanation: str = Field(..., description="Detailed text explanation of impact and costs")
...
class AttritionStep(BaseModel):
    ...
    passed_count: int = Field(..., description="Number of patients passing this criterion")
    failed_count: int = Field(..., description="Number of patients failing this criterion")
    remaining_count: int = Field(..., description="Number of patients continuing to the next step")
    attrition_rate: float = Field(..., description="Percentage of current cohort lost at this step")
...
class FeasibilityReport(BaseModel):
    ...
    final_eligible_count: int = Field(..., description="Number of fully eligible patients")
    overall_eligibility_rate: float = Field(..., description="Percentage of cohort that is eligible")
    attrition_steps: List[AttritionStep] = Field(default_factory=list, description="Step-by-step funnel of attrition")
...
class ProtocolQualityScore(BaseModel):
    ...
    readability: Optional[ReadabilityReport] = Field(None, description="Readability metrics of narrative text blocks")
    burden_details: Optional[BurdenTraceReport] = Field(None, description="Traceable operational burden details")
    amendment_impact: Optional[AmendmentImpactReport] = Field(None, description="Amendment impact and cost estimation")
    feasibility: Optional[FeasibilityReport] = Field(None, description="Pluggable patient population feasibility metrics")
