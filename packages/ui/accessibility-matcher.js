...
export async function toBeAccessible(received, customOptions = {}) {
  ...
  if (
    customOptions.strictLayoutMode ||
    (customOptions.rules && customOptions.rules["landmark-one-main"]?.enabled)
  ) {
    const hasMain = element.querySelector("main#main-content");
    const hasSkipLink = element.querySelector(
    ...
    if (!hasMain || !hasSkipLink) {
      return {
        ...
        message: () =>
          ...
          (!hasMain
            ? ` - Missing main landmark element (<main>) with id="main-content"\n`
            : "") +
          (!hasSkipLink
            ? ` - Missing skip-to-content link (.skip-link) with href="#main-content"\n`
            : ""),
      };
    }
  }
