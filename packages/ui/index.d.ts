...
export const ClinicalFormField: DefineComponent<{
  ...
  query?: any;
  error?: string | null;
  lookupStatus?: { status: "none" | "loading" | "valid" | "invalid" | "degraded"; message?: string } | null;
}, {}, any>;
...
export function createConditionRowHTML(index: number, forms?: any[], fields?: any[], initialData?: any): string;
export function createRuleEditorHTML(forms?: any[], fields?: any[], options?: any): string;
export function serializeConditionsTree(conditions?: any[], matchOperator?: string): any;
export function deserializeConditionsTree(tree: any): { conditions: any[]; matchOperator: string };
export function createSoaBuilderMatrix(soaData: any): string;
export function createClinicalVisitMatrix(visitsOrSoa: any, forms?: any[]): string;
...
export function verifyCanonicalSignature(obj: any, sig: string, key: any): boolean;
export function generateGatewaySignature(payload: any, key: any): string;
export function verifyGatewaySignature(payload: any, sig: string, key: any): boolean;
...
export function buildLedgerBlock(index: number, prevHash: string, data: any, key: any): Promise<any>;
export function encryptAESGCM(plaintext: string, key: Uint8Array): Promise<{ ciphertext: string; iv: string; tag: string }>;
export function decryptAESGCM(ciphertext: string, iv: string, tag: string, key: Uint8Array): Promise<string>;
export function deriveSessionKey(pin: string, salt: string): Promise<Uint8Array>;
export function deriveKeyFromPIN(pin: string, salt: string): Promise<Uint8Array>;
