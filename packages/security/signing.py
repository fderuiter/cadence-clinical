...
def verify_gateway_signature(
    ...
    # We treat "tenant_default" as a legacy/default tenant fallback that does not block legacy signature verifications.
    has_scopes = bool(site_id or sponsor_id or unblinded_access or (tenant_id and tenant_id != "tenant_default"))
    if not has_scopes:
